...
main(int argc, char **argv) {
    ...
    if (ret < 0 || ret > (PATHLEN - 2)) {
        fprintf(stderr, "Path to executable too long: %s/bin/%s", base, basename(buf));
        return 1;
    }
    memset(lib, 0, PATHLEN);
    ret = snprintf(lib, PATHLEN, "%s/lib", base);
    if (ret < 0 || ret > (PATHLEN - 2)) {
        fprintf(stderr, "Path to lib too long: %s/lib", base);
        return 1;
    }
    ...
    if (ldp == NULL) strncpy(buf, lib, PATHLEN);
        ...
        if (ret < 0 || ret > (PATHLEN - 2)) {
            fprintf(stderr, "LD_LIBRARY_PATH too long: %s:%s", lib, ldp);
            return 1;
        }
    }
    ...
    if (ret < 0 || ret > (PATHLEN - 2)) {
        fprintf(stderr, "OPENSSL_MODULES too long: %s/ossl-modules", lib);
        return 1;
    }
