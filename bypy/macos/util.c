...
set_env_vars(const char *contents_path) {
    ...
    for (size_t i = 0; i < arraysz(env_vars); i++) {
        env_var = env_vars[i];
        val = env_var_vals[i];
        if (strcmp(env_var, "SSL_CERT_DIR") == 0 && getenv("CALIBRE_USE_SYSTEM_CERTIFICATES") && strcmp(getenv("CALIBRE_USE_SYSTEM_CERTIFICATES"), "1") == 0)
            continue;
...
run(const wchar_t *program, const wchar_t *module, const wchar_t *function, bool gui_app, int argc, char *const *argv, char *exe_path) {
    ...
    interpreter_data.basename = program;
    interpreter_data.module = module;
    interpreter_data.function = function;
