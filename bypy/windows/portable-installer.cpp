...
temp_file(LPWSTR name) {
    ...
    if (res == 0) {
        show_last_error(L"Failed to create temporary file to decompress portable data");
...
static BOOL
decompress(LPVOID src, DWORD src_sz, HANDLE out, IProgressDialog *pd) {
    ...
    rc = elzma_decompress_run(h, input_callback, (void *)&ds, output_callback, (void *)&ds, ELZMA_lzip);
...
make_unpack_dir() {
    ...
    if (directory_exists(L"_unpack_calibre_portable")) rmtree(L"_unpack_calibre_portable");
...
static BOOL
move_program() {
    if (MoveFileEx(L"Calibre Portable\\calibre-portable.exe", L"..\\calibre-portable.exe", MOVEFILE_REPLACE_EXISTING) == 0) {
        show_last_error(L"Failed to move calibre-portable.exe, make sure calibre is not running");
        return false;
    }
    if (MoveFileEx(L"Calibre Portable\\ebook-viewer-portable.exe", L"..\\ebook-viewer-portable.exe", MOVEFILE_REPLACE_EXISTING) == 0) {
        show_last_error(L"Failed to move ebook-viewer-portable.exe, make sure calibre is not running");
        return false;
    }
    if (MoveFileEx(L"Calibre Portable\\ebook-edit-portable.exe", L"..\\ebook-edit-portable.exe", MOVEFILE_REPLACE_EXISTING) == 0) {
    ...
    if (!directory_exists(L"..\\Calibre Library")) { MoveFileEx(L"Calibre Portable\\Calibre Library", L"..\\Calibre Library", 0); }

    if (!directory_exists(L"..\\Calibre Settings")) { MoveFileEx(L"Calibre Portable\\Calibre Settings", L"..\\Calibre Settings", 0); }
...
launch_calibre() {
    ...
    if (CreateProcess(_wcsdup(L"calibre-portable.exe"), NULL, NULL, NULL, FALSE, CREATE_UNICODE_ENVIRONMENT | CREATE_NEW_PROCESS_GROUP, NULL, NULL, &si, &pi) ==
        0) {
...
wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PWSTR pCmdLine, int nCmdShow) {
    ...
    do {
        ...
        if (wcslen(fdest) > 58) {
            _snwprintf_s(buf, 4 * MAX_PATH, _TRUNCATE, L"Path to Calibre Portable (%s) too long. It must be less than 59 characters.", fdest);
    ...
    if (existing && !automated) {
        _snwprintf_s(mb_msg, 4 * MAX_PATH, _TRUNCATE, L"An existing install of Calibre Portable was found at %s. Do you want to upgrade it?", fdest);
        if (MessageBox(NULL, mb_msg, L"Upgrade Calibre Portable?", MB_ICONEXCLAMATION | MB_YESNO | MB_TOPMOST) != IDYES) goto end;
    }
    ...
    ret = 0;
    if (!automated) {
        _snwprintf_s(mb_msg, 4 * MAX_PATH, _TRUNCATE, L"Calibre Portable successfully installed to %s. Launch calibre?", fdest);
