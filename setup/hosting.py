...
class ReadFileWithProgressReporting:  # {{{
    ...
    def report_progress(self, size):
        ...
        sys.stdout.write(f'  {frac * 100:.1f}%   {mb_pos:.1f}/{mb_tot:.1f}MB {kb_rate:.1f} KB/sec    {eta_m} minutes, {eta_s} seconds left')
        sys.stdout.write('\x1b[u')
        if self.tell() >= self._total:
            sys.stdout.write('\n')
            t = int(time.time() - self.start_time) + 1
            print(f'Upload took {t / 60} minutes and {t % 60} seconds at {kb_rate:.1f} KB/sec')
        sys.stdout.flush()
        ...
        for release in releases:
            if release.get('assets', None) and release['tag_name'] != self.current_tag_name:
                ...
                for asset in release['assets']:
                    r = self.requests.delete(self.API + 'repos/{}/{}/releases/assets/{}'.format(self.username, self.reponame, asset['id']))
                    if r.status_code != 204:
                        self.fail(r, 'Failed to delete obsolete asset: {} for release: {}'.format(asset['name'], release['tag_name']))
...
def generate_index():  # {{{
    ...
    for series in rmap:
        body.append(
            '<li><a href="{0}.html" title="Releases in the {0}.x series">{0}.x</a>\xa0\xa0\xa0<span style="font-size:smaller">[{1} releases]</span></li>'.format(  # noqa: E501
    ...
    for series, releases in rmap.items():
        ...
        index = template.format(title=f'Previous calibre releases ({sname}.x)', style=style, msg='Choose a calibre release', body=body)
        ...
        for r in releases:
            ...
            try:
                ...
                linux = [x for x in files if x.endswith(('.txz', 'tar.bz2'))]
                if linux:
                ...
                index = template.format(title=f'calibre release ({rname})', style=style, msg='', body=body)
                with open('index.html', 'wb') as f:
                    f.write(index.encode('utf-8'))
            finally:
                os.chdir('..')
