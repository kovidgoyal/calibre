...
import operator, os, json, re, logging
...
class BrowseServer(object):
    ...
    def add_routes(self, connect):
        ...
        connect('browse_category_icon', base_href+'/icon/{name}',
                self.browse_icon)
        # v2
        connect('browse_list_all', '/v2/list/all/{page}',
                self.list_category_all)
        connect('book_detail', '/v2/book/{book_id}',
                self.book_detail)

    @Endpoint(sort_type='list')
    def list_category_all(self, page=0, list_sort=None):
        delta = 20
        try: page = int(page)
        except: page = 0
        ...
        items = self.db.get_data_as_dict(ids=ids)[page:page+delta]
        url_prfix = self.opts.url_prefix
        M = "/static/v2/m"
        template = T('content_server/v2/list.html')
        return template.render(vars(), db=self.db)

    @Endpoint(sort_type='book')
    def book_detail(self, book_id, book_sort=None):
        book_id = int(book_id)
        ...
        url_prfix = self.opts.url_prefix
        M = "/static/v2/m"
        template = T('content_server/v2/book.html')
        return template.render(vars(), db=self.db)
