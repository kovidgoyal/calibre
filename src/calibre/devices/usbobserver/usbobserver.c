...
usbobserver_get_bsd_path(io_object_t dev) {
    ...
    if (!CFStringGetCString(PropRef, cpath + dev_path_length, MAXPATHLEN - dev_path_length, kCFStringEncodingUTF8)) return NULL;
...
usbobserver_get_mounted_filesystems(PyObject *self, PyObject *args) {
    ...
    for (i = 0; i < num; i++) {
        ...
        if (PyDict_SetItemString(ans, buf[i].f_mntfromname, val) != 0) {
            NUKE(ans);
            NUKE(val);
            goto end;
        }
...
usbobserver_is_mtp(PyObject *self, PyObject *args) {
    ...
    Py_BEGIN_ALLOW_THREADS num = CFNumberCreate(kCFAllocatorDefault, kCFNumberSInt32Type, &vendor_id);
    if (num == NULL) {
        PyErr_NoMemory();
        goto end;
    }
    ...
    Py_END_ALLOW_THREADS if (matching_dict) CFRelease(matching_dict);
...
static PyMethodDef usbobserver_methods[] = {
    {"get_usb_devices",
     usbobserver_get_usb_devices,
     METH_VARARGS,
     "Get list of connected USB devices. Returns a list of tuples. Each tuple is of the form (vendor_id, product_id, bcd, manufacturer, product, serial "
     "number)."},
    {"get_usb_drives", usbobserver_get_usb_drives, METH_VARARGS, "Get list of mounted drives. Returns a list of tuples, each of the form (name, bsd_path)."},
    {"get_mounted_filesystems",
     usbobserver_get_mounted_filesystems,
     METH_VARARGS,
     "Get mapping of mounted filesystems. Mapping is from BSD name to mount point."},
    {"user_locale", usbobserver_user_locale, METH_VARARGS, "user_locale() -> The name of the current user's locale or None if an error occurred"},
    {"date_format", usbobserver_date_fmt, METH_VARARGS, "date_format() -> The (short) date format used by the user's current locale"},
    {"is_mtp_device",
     usbobserver_is_mtp,
     METH_VARARGS,
