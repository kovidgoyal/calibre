    ...
    def __init__(self, parent, device):
        ...
        self.la = la = QLabel(
            _(
                ...
                'these are removed from all styles in the original stylesheet.'
            ).format(device.KOBO_EXTRA_CSSFILE),
        )
        la.setWordWrap(True)
        l.addWidget(la)
        self.la2 = la = QLabel(_('Additionally, model specific CSS can be specified below:'))
        la.setWordWrap(True)
        l.addWidget(la)
        ...
        l.addLayout(h)
        for mm in [ModelMetadata('', _('All models'), -1, -1, -1, type(device))] + sorted(device.model_metadata(), key=lambda x: x.model_name.lower()):
...
class BookUploadsGroupBox(DeviceOptionsGroupBox):
    def __init__(self, parent, device):
        ...
        self.kepubify_checkbox = create_checkbox(
            _('Use Kobo viewer for EPUB books'),
            _(
        ...
        self.kepubify_template_edit = TemplateConfig(
            ...
            + _(
                ...
                ' or to only convert books that do not have the tag <i>{2}</i>, use the template: <code>{3}</code>'
            ).format(
                'as_kepub',
                r'{tags:str_in_list(\,,as_kepub,true,false)}',
                'as_epub',
                r'{tags:str_in_list(\,,as_epub,false,true)}',
            )
            + '<p>'
            + _(
        ...
        self.override_kobo_replace_existing_checkbox = create_checkbox(
            ...
            _(
                'When a new book is side-loaded, the Kobo firmware imports details of the book into the internal database. '
...
class CollectionsGroupBox(DeviceOptionsGroupBox):
    def __init__(self, parent, device):
        ...
        self.collections_columns_edit.setToolTip(
            '<p>'
            + _(
                'The Kobo from firmware V2.0.0 supports collections. '
                'These are created on the Kobo. Specify the lookup name for a '
                'tags-type column for automatic management.'
            )
            + '</p>'
        )
        ...
        self.collections_template_edit = TemplateConfig(
            ...
            tooltip='<p>'
            + _(
                "Enter a template to generate collections. "
        ...
        self.ignore_collections_names_edit.setToolTip(
            '<p>'
            + _(
                'List the names of collections to be ignored by '
    ...
    def validate_collections_columns(self):
        ...
        if bad_names:
            ...
            error_dialog(
                self,
                _('Kobo configuration: Invalid collection column names'),
...
class CoversGroupBox(DeviceOptionsGroupBox):
    def __init__(self, parent, device):
        ...
        self.dithered_covers_checkbox = create_checkbox(
            ...
            _(
                'Dither cover images to the appropriate 16c grayscale palette for an eInk screen.'
        ...
        self.keep_cover_aspect_checkbox = create_checkbox(
            ...
            _('When uploading covers, do not change the aspect ratio when resizing for the device. This is for firmware versions 2.3.1 and later.'),
        ...
        self.png_covers_checkbox = create_checkbox(
            ...
            _(
                'Use the PNG image format instead of JPG.'
...
class AdvancedGroupBox(DeviceOptionsGroupBox):
    def __init__(self, parent, device):
        ...
        self.support_newer_firmware_checkbox = create_checkbox(
            ...
            + str(device.supported_dbversion),
...
class MetadataGroupBox(DeviceOptionsGroupBox):
    def __init__(self, parent, device):
        ...
        self.setToolTip(
            wrap_msg(
                _(
                    'Update the metadata on the device when it is connected. '
        ...
        self.force_series_id_checkbox = create_checkbox(
            _('Force series ID'),
            _(
                'Kobo devices use a SeriesID to distinguish between different series. '
        ...
        self.series_index_template_edit = TemplateConfig(
            ...
            tooltip='<p>'
            + _(
                'The Kobo can display a series number (book number) '
                ...
                'up the series in calibre.'
            ).format('<pre style="white-space: nowrap;">program: if $series_index !=# 0 then $series_index else "" fi</pre>')
            + '</p>',
        )
        self.update_core_metadata_checkbox = create_checkbox(
            _('Update metadata on Book Details pages'),
            _(
                'This will update the metadata in the device database when the device is connected. '
        ...
        self.update_subtitle_checkbox = create_checkbox(
            _('Subtitle'), _('Update the subtitle on the device using a template.'), device.get_pref('update_subtitle')
        )
        self.subtitle_template_edit = TemplateConfig(
            self.update_subtitle_checkbox.text(),
            device.get_pref('subtitle_template'),
            tooltip=_('Enter a template to use to set the subtitle. If the template is empty, the subtitle will be cleared.'),
        )
        ...
        self.bookstats_wordcount_template_edit = TemplateConfig(
            ...
            label=_('Words:'),
            tooltip=_('Enter a template to use to set the word count for the book. If the template is empty, the word count will be cleared.'),
        )
        self.bookstats_pagecount_template_edit = TemplateConfig(
            _('Pages'),
            device.get_pref('bookstats_pagecount_template'),
            label=_('Pages:'),
            tooltip=_('Enter a template to use to set the page count for the book. If the template is empty, the page count will be cleared.'),
        )
        ...
        self.bookstats_timetoread_upper_template_edit = TemplateConfig(
            ...
            label=_('Upper:'),
            tooltip=_(
                'Enter a template to use to set the upper estimate of the time to read for the book. '
        ...
        self.bookstats_timetoread_lower_template_edit = TemplateConfig(
            ...
            label=_('Lower:'),
            tooltip=_(
                'Enter a template to use to set the lower estimate of the time to read for the book. '
                'The estimate is in hours. '
                'If the template is empty, the time will be cleared.'
            ),
        )
...
class TemplateConfig(QWidget):  # {{{
    ...
    def validate(self):
        ...
        try:
            ...
            error_dialog(
                self,
                _('Invalid template for {0}').format(self.name),
