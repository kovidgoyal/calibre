...
class MTP_DEVICE(MTPDeviceBase):
    ...
    def detect_managed_devices(self, devices_on_system, force_refresh=False):
        ...
        for dev in tuple(six.iterkeys(self.detected_devices)):
        ...
        for dev, data in six.iteritems(self.detected_devices):
    ...
    def filesystem_cache(self):
        if self._filesystem_cache is None:
            ...
            for storage_id, capacity in zip([self._main_id, self._carda_id,
                ...
                for x in six.itervalues(id_map):
                    x['storage_id'] = storage_id
                all_storage.append(storage)
                items.append(six.itervalues(id_map))
