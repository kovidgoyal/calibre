...
open_device(const wchar_t *pnp_id, CComPtr<IPortableDeviceValues> &client_information) { // {{{
    ...
    if (FAILED(hr)) {
        hresult_set_exc("Failed to create IPortableDevice", hr);
        device = NULL;
    } else {
        Py_BEGIN_ALLOW_THREADS;
...
get_storage_info(IPortableDevice *device) { // {{{
    ...
    Py_END_ALLOW_THREADS;
    if (FAILED(hr)) {
        hresult_set_exc("Failed to get content interface from device", hr);
        return NULL;
    }
    ...
    Py_END_ALLOW_THREADS;
    if (FAILED(hr)) {
        hresult_set_exc("Failed to get properties interface", hr);
        return NULL;
    }
    ...
    Py_END_ALLOW_THREADS;
    if (FAILED(hr)) {
        hresult_set_exc("Failed to create storage properties collection", hr);
        return NULL;
    }

#define A(what)                                                                            \
    hr = storage_properties->Add(what);                                                    \
    if (FAILED(hr)) {                                                                      \
        hresult_set_exc("Failed to add storage property " #what " for storage query", hr); \
        return NULL;                                                                       \
    }
...
#undef A
    ...
    Py_END_ALLOW_THREADS;
    if (FAILED(hr)) {
        hresult_set_exc("Failed to get objects from device", hr);
        return NULL;
    }
    ...
    while (hr == S_OK) {
        ...
        if (SUCCEEDED(hr)) {
            for (i = 0; i < fetched; i++) {
                ...
                Py_END_ALLOW_THREADS;
                if (SUCCEEDED(hr2)) {
                    if (SUCCEEDED(values->GetGuidValue(WPD_OBJECT_CONTENT_TYPE, &guid)) && IsEqualGUID(guid, WPD_CONTENT_TYPE_FUNCTIONAL_OBJECT) &&
                        ...
                        if (SUCCEEDED(values->GetUnsignedIntegerValue(WPD_STORAGE_ACCESS_CAPABILITY, &access)) &&
                            access == WPD_STORAGE_ACCESS_CAPABILITY_READWRITE)
                            paccess = Py_True;
                        pyobject_raii soid(PyUnicode_FromWideChar(object_ids[i], -1));
                        if (!soid) return NULL;
                        pyobject_raii so(Py_BuildValue(
                            "{s:K, s:K, s:K, s:K, s:O, s:O}",
                            "capacity",
                            capacity,
                            "capacity_objects",
                            capacity_objects,
...
#define A(which, key)                                                         \
    {                                                                         \
        com_wchar_raii buf;                                                   \
        if (SUCCEEDED(values->GetStringValue(which, buf.unsafe_address()))) { \
...
#undef A
                        const wchar_t *st;
                        switch (storage_type) {
                            case WPD_STORAGE_TYPE_REMOVABLE_RAM: st = L"removable_ram"; break;
                            case WPD_STORAGE_TYPE_REMOVABLE_ROM: st = L"removable_rom"; break;
                            case WPD_STORAGE_TYPE_FIXED_RAM: st = L"fixed_ram"; break;
                            case WPD_STORAGE_TYPE_FIXED_ROM: st = L"fixed_rom"; break;
                            default: st = L"unknown_unknown";
                        }
...
#define A(what)           \
    hr = keys->Add(what); \
    if (FAILED(hr)) { return hresult_set_exc("Failed to add key " #what " to IPortableDeviceKeyCollection", hr); }
...
#define S(what, key)                                                             \
    {                                                                            \
        ...
        if (SUCCEEDED(values->GetStringValue(what, temp.unsafe_address()))) {    \
            ...
            if (t)                                                               \
                if (PyDict_SetItemString(ans, key, t.ptr()) != 0) PyErr_Clear(); \
        }                                                                        \
    }
    ...
    if (SUCCEEDED(values->GetUnsignedIntegerValue(WPD_DEVICE_TYPE, &ti))) {
        type = "unknown";
        switch (ti) {
            case WPD_DEVICE_TYPE_CAMERA: type = "camera"; break;
            case WPD_DEVICE_TYPE_MEDIA_PLAYER: type = "media player"; break;
            case WPD_DEVICE_TYPE_PHONE: type = "phone"; break;
            case WPD_DEVICE_TYPE_VIDEO: type = "video"; break;
            case WPD_DEVICE_TYPE_PERSONAL_INFORMATION_MANAGER: type = "personal information manager"; break;
            case WPD_DEVICE_TYPE_AUDIO_RECORDER: type = "audio recorder"; break;
            case WPD_DEVICE_TYPE_GENERIC: break;
        }
