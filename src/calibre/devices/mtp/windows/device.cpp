...
get_file_by_name(Device *self, PyObject *args) {
    ...
    for (Py_ssize_t i = 0; i < PyTuple_GET_SIZE(names); i++) {
        ...
        pyobject_raii l(PyObject_CallMethod(k, "lower", NULL));
        if (!l) return NULL;
    ...
    if (!found) {
        PyErr_SetString(PyExc_KeyError, "File not found");
        return NULL;
    }
    if (callback == NULL || !PyCallable_Check(callback)) callback = NULL;
...
static PyMethodDef Device_methods[] = {
    ...
    {"get_filesystem",
     ...
     "callback must be a callable that is called as (object, level). It is called with every found object. If the callback returns False and the object is a "
     "folder, it is not recursed into."},

    {"list_folder_by_name",
     (PyCFunction)list_folder_by_name,
     METH_VARARGS,
     "list_folder_by_name(parent_id, names) -> List the folder specified by names (a tuple of name components) relative to parent_id from the device. Return "
     "None or a list of entries."},

    {"get_metadata_by_name",
     (PyCFunction)get_metadata_by_name,
     METH_VARARGS,
    ...
    {"get_file",
     (PyCFunction)py_get_file,
     METH_VARARGS,
     "get_file(object_id, stream, callback=None) -> Get the file identified by object_id from the device. The file is written to the stream object, which must "
     "be a file like object. If callback is not None, it must be a callable that accepts two arguments: (bytes_read, total_size). It will be called after each "
     "chunk is read from the device. Note that it can be called multiple times with the same values."},

    {"get_file_by_name",
     (PyCFunction)get_file_by_name,
     METH_VARARGS,
    ...
    {"create_folder", (PyCFunction)py_create_folder, METH_VARARGS, "create_folder(parent_id, name) -> Create a folder. Returns the folder metadata."},

    {"delete_object",
     (PyCFunction)py_delete_object,
     METH_VARARGS,
    ...
    {"put_file",
     (PyCFunction)py_put_file,
     METH_VARARGS,
     "put_file(parent_id, name, stream, size_in_bytes, callback=None) -> Copy a file from the stream object, creating a new file on the device with parent "
     "identified by parent_id. Returns the file metadata of the newly created file. callback should be a callable that accepts two argument: (bytes_written, "
     "total_size). It will be called after each chunk is written to the device. Note that it can be called multiple times with the same arguments."},
...
static PyGetSetDef Device_getsetters[] = {
    {(char *)"data", (getter)Device_data, NULL, (char *)"The basic device information.", NULL},
