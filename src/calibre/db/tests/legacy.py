        ...
        if callable(meth):
            ...
            if meth[0] in {'!', '@', '#', '+', '$', '-', '%'}:
                if meth[0] != '+':
                    fmt = {
                        '!': dict,
                        '@': lambda x: frozenset(x or ()),
                        '#': lambda x: set((x or '').split(',')),
                        '$': lambda x: {tuple(y) for y in x},
                        '-': lambda x: None,
                        '%': lambda x: set((x or '').split(',')),
                    }[meth[0]]
                else:
...
class LegacyTest(BaseTest):
    ...
    def test_library_wide_properties(self):  # {{{
        ...
        def to_unicode(x):
            ...
            if isinstance(x, dict):
                ...
                return {k.decode('utf-8') if isinstance(k, bytes) else k: to_unicode(v) for k, v in x.items() if k != 'rec_index'}
            return x
    ...
    def test_get_property(self):  # {{{
        ...
        def get_values(db):
            ...
            for label, loc in db.FIELD_MAP.items():
                ...
                label = str(label)
                ans[label] = tuple(db.get_property(i, index_is_id=True, loc=loc) for i in db.all_ids())
    ...
    def test_legacy_direct(self):  # {{{
        ...
        for meth, args in {
            ...
            for a in args:
                self.assertEqual(
                    fmt(getattr(db, meth)(*a)),
                    fmt(getattr(ndb, meth)(*a)),
    ...
    def test_legacy_conversion_options(self):  # {{{
        ...
        for x in (
            ...
            meth, args = x[0], x[1:]
            self.assertEqual(
                decode(getattr(db, meth)(*args)),
                decode(getattr(ndb, meth)(*args)),
    ...
    def test_legacy_coverage(self):  # {{{
        ...
        SKIP_ATTRS = {
            ...
            'books_in_old_database',
            'sizeof_old_database',  # unused
            ...
            'dirtied_lock',
            'dirtied_cache',
            'dirty_books_referencing',
            'windows_check_if_files_in_use',
            'get_metadata_for_dump',
            'get_a_dirtied_book',
            'dirtied_sequence',
            'format_filename_cache',
            'format_metadata_cache',
            'filter',
            'create_version1',
            'normpath',
            'custom_data_adapters',
            'custom_table_names',
    ...
    def test_legacy_custom(self):  # {{{
        ...
        run_funcs(
            ...
            (
                ...
                ('all_custom', 'authors'),
                ('all_custom', None, 7),
                ('get_next_cc_series_num_for', 'My Series One', 'series'),
                ('get_next_cc_series_num_for', 'My Series Two', 'series'),
                ('is_item_used_in_multiple', 'My Tag One', 'tags'),
                ('is_item_used_in_multiple', 'My Series One', 'series'),
                ('$get_custom_items_with_ids', 'series'),
                ('$get_custom_items_with_ids', 'tags'),
                ('$get_custom_items_with_ids', 'float'),
                ('$get_custom_items_with_ids', 'rating'),
                ('$get_custom_items_with_ids', 'authors'),
                ('$get_custom_items_with_ids', None, 7),
            ),
        )
        for label in (
            'tags',
