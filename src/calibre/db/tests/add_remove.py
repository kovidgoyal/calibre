...
class AddRemoveTest(BaseTest):
    ...
    def test_remove_books(self):  # {{{
        ...
        self.assertIn(_('Unknown'), set(six.itervalues(authors.id_map)))
        olen = len(authors.id_map)
        item_id = {v:k for k, v in six.iteritems(authors.id_map)}[_('Unknown')]
        cache.remove_books((3,))
        for c in (cache, self.init_cache()):
            table = c.fields['authors'].table
            self.assertNotIn(3, c.all_book_ids())
            self.assertNotIn(_('Unknown'), set(six.itervalues(table.id_map)))
        ...
        item_id = {v:k for k, v in six.iteritems(cache.fields['#series'].table.id_map)}['My Series Two']
        ...
        for c in (cache, self.init_cache()):
            ...
            self.assertNotIn('Author Two', set(six.itervalues(table.id_map)))
            self.assertNotIn(6, set(six.itervalues(c.fields['rating'].table.id_map)))
            self.assertIn('A Series One', set(six.itervalues(c.fields['series'].table.id_map)))
            self.assertNotIn('My Series Two', set(six.itervalues(c.fields['#series'].table.id_map)))
