...
class LibraryDatabase(object):
    ...
    def get_usage_count_by_id(self, field):
        return [[k, v] for k, v in six.iteritems(self.new_api.get_usage_count_by_id(field))]

    def field_id_map(self, field):
        return [(k, v) for k, v in six.iteritems(self.new_api.get_id_map(field))]

    def get_custom_items_with_ids(self, label=None, num=None):
        try:
            return [[k, v] for k, v in six.iteritems(self.new_api.get_id_map(self.custom_field_name(label, num)))]
        except ValueError:
            return []
    ...
    def add_books(self, paths, formats, metadata, add_duplicates=True, return_ids=False):
        ...
        if duplicates:
            ...
            for mi, format_map in duplicates:
                metadata.append(mi)
                for fmt, path in six.iteritems(format_map):
                    formats.append(fmt)
                    paths.append(path)
    ...
    def books_with_same_title(self, mi, all_matches=True):
        ...
        if title:
            ...
            for book_id, x in six.iteritems(self.new_api.get_id_map('title')):
    ...
    def delete_tags(self, tags):
        with self.new_api.write_lock:
            tag_map = {icu_lower(v):k for k, v in six.iteritems(self.new_api._get_id_map('tags'))}
    ...
    def format_files(self, index, index_is_id=False):
        book_id = index if index_is_id else self.id(index)
        return [(v, k) for k, v in six.iteritems(self.new_api.format_files(book_id))]
    ...
    def delete_item_from_multiple(self, item, label=None, num=None):
        ...
        rmap = {icu_lower(v):k for k, v in six.iteritems(existing)}
...
LibraryDatabase.update_last_modified = \
...
for func, field in six.iteritems({'all_authors':'authors', 'all_titles':'title', 'all_tags2':'tags', 'all_series':'series', 'all_publishers':'publisher'}):
    def getter(field):
        def func(self):
...
LibraryDatabase.get_authors_with_ids = \
    lambda self: [[aid, adata['name'], adata['sort'], adata['link']] for aid, adata in six.iteritems(self.new_api.author_data())]
LibraryDatabase.get_author_id = \
    lambda self, author: {icu_lower(v):k for k, v in six.iteritems(self.new_api.get_id_map('authors'))}.get(icu_lower(author), None)
...
for field in ('tags', 'series', 'publishers', 'ratings', 'languages'):
    def getter(field):
        ...
        def func(self):
            return [[tid, tag] for tid, tag in six.iteritems(self.new_api.get_id_map(fname))]
        return func
