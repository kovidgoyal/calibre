...
class PluginWidget(Widget, Ui_Form):
    ...
    def __init__(self, parent, get_option, get_help, db=None, book_id=None):
        ...
        self.md_map = {}
        for name, text in six.iteritems(MD_EXTENSIONS):
    ...
    def set_value_handler(self, g, val):
        if g is self.opt_markdown_extensions:
            for i in six.itervalues(self.md_map):
    ...
    def get_value_handler(self, g):
        ...
        return ', '.join(unicode(i.data(Qt.UserRole) or '') for i in six.itervalues(self.md_map) if i.checkState())
