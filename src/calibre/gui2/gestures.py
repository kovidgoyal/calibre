...
class TouchPoint:
    ...
    def update(self, tp):
        ...
        self.previous_screen_position, self.current_screen_position = (
            self.current_screen_position,
...
class GestureManager(QObject):
    def __init__(self, view):
        ...
        self.evmap = {
            ...
            QEvent.Type.TouchCancel: 'cancel',
        }
        self.last_tap_at = 0
        if touch_supported():
    ...
    def handle_event(self, ev):
        ...
        if etype in (
            ...
            scroller.stop()
            return
        if etype == QEvent.Type.Wheel and scroller.state() != QScroller.State.Inactive:
            ev.ignore()
            return False
    ...
    def handle_flicking(self, touch_point, is_end):
        ...
        scroller = self.scroller
        assert scroller is not None
        scroller.handleInput(it, touch_point.current_position, int(touch_point.last_update_time * 1000))
