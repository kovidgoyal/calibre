...
QProgressIndicator::QProgressIndicator(QWidget *parent, int size, int interval) : QWidget(parent), m_displaySize(size, size), m_animator(this) {
    Q_UNUSED(interval);
...
void
QProgressIndicator::setDisplaySize(QSize size) {
    setSizeHint(size);
}
void
QProgressIndicator::setSizeHint(int size) {
    setSizeHint(QSize(size, size));
}
void
QProgressIndicator::setSizeHint(QSize size) {
    m_displaySize = size;
    update();
}
...
void
set_no_activate_on_click(QWidget *widget) {
    QStyle *base_style = widget->style();
    if (base_style) widget->setStyle(new NoActivateStyle(base_style));
}

void
draw_snake_spinner(QPainter &painter, QRect rect, int angle, const QColor &light, const QColor &dark) {
    painter.save();
...
luminance(const QColor &color) {
    return 0.2126 * channelToLinear(color.redF()) + 0.7152 * channelToLinear(color.greenF()) + 0.0722 * channelToLinear(color.blueF());
}
...
class Translator : public QTranslator {
    QString
    translate(const char *context, const char *text, const char *disambiguation = nullptr, int n = -1) const override {
        (void)context;
        (void)disambiguation;
        (void)n;
