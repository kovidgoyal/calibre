    ...
    def start_test(self, *args):
        if not self.to.text().strip():
            return error_dialog(
                self,
                _('No email address'),
                _('No email address to send mail to has been specified. You must specify a To: address before running the test.'),
                show=True,
            )
...
class RelaySetup(QDialog):
    def __init__(self, service, parent):
        ...
        self.tl = QLabel(
            (
                ...
                + ' <b>{name}</b><p>'
                + _("If you don't have an account, you can sign up for a free {name} email account at {link_markup}. {extra}")
            ).format(**fields)
        )
        ...
        self.bl = QLabel(
            ...
            + _(
                ...
                'Amazon.com Kindle management page.'
            )
            % service['name']
        )
    ...
    def accept(self):
        ...
        if not pw:
            return error_dialog(self, _('No password'), _('You must specify a password'), show=True)
        fr = self.from_.text().strip()
        if not fr:
            return error_dialog(self, _('No email address'), _('You must specify an email address'), show=True)
        un = self.service_username
        if not un:
            return error_dialog(
                self,
                _('Incorrect email address'),
                _('The email address "{}" is not valid').format(self.from_.text()),
                show=True,
            )
        QDialog.accept(self)
...
class SendEmail(QWidget, Ui_Form):
    ...
    def initialize(self, preferred_to_address):
        ...
        self.relay_show_password.stateChanged.connect(
            lambda state: self.relay_password.setEchoMode(QLineEdit.EchoMode.Password if state == 0 else QLineEdit.EchoMode.Normal)
        )
    ...
    def create_service_relay(self, service, *args):
        rservice = {
            'gmx': {
                ...
                'url': 'www.gmx.com',
                'extra': _(
                    'Before using this account to send mail, you must enable the'
            ...
            'gmail': {
                ...
                'extra': _(
                    ...
                    ' <a href="{}">here</a>.'
                ).format('https://support.google.com/accounts/answer/6010255'),
                'at_in_username': True,
            },
            'hotmail': {
                'name': 'Outlook',
    ...
    def set_email_settings(self, to_set):
        ...
        if to_set and not from_:
            error_dialog(self, _('Bad configuration'), _('You must set the From email address')).exec()
            return False
        ...
        if host:
            ...
            if (username and not password) or (not username and password):
                error_dialog(
                    self,
                    _('Bad configuration'),
                    _('You must either set both the username <b>and</b> password for the mail server or no username and no password at all.'),
                ).exec()
                return False
            if not (username and password) and not question_dialog(
                self,
                _('Are you sure?'),
                _('No username and password set for mailserver. Most  mailservers need a username and password. Are you sure?'),
            ):
                return False
        conf = smtp_prefs()
