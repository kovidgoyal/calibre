...
class ConvertAction(InterfaceActionWithLibraryDrop):
    ...
    def genesis(self):
        ...
        cm(
            'convert-individual',
            _('Convert individually'),
            icon=self.qaction.icon(),
            triggered=partial(self.convert_ebook, False, bulk=False),
        )
        cm('convert-bulk', _('Bulk convert'), triggered=partial(self.convert_ebook, False, bulk=True))
        m.addSeparator()
        cm(
            'create-catalog',
    ...
    def auto_convert(self, book_ids, on_card, format):
        ...
        if jobs == []:
            return
        self.queue_convert_jobs(jobs, changed, bad, rows, previous, self.book_auto_converted, extra_job_args=[on_card])
    ...
    def auto_convert_auto_add(self, book_ids):
        ...
        for book_id in book_ids:
            ...
            if gprefs['auto_convert_same_fmt'] or of not in fmts:
                needed.add(book_id)
        if needed:
            jobs, changed, bad = convert_single_ebook(self.gui, self.gui.library_view.model().db, needed, True, of, show_no_format_warning=False)
            if not jobs:
                return
    ...
    def auto_convert_catalogs(self, book_ids, format):
        ...
        if jobs == []:
            return
        self.queue_convert_jobs(jobs, changed, bad, rows, previous, self.book_auto_converted_catalogs)

    def get_books_for_conversion(self):
        rows = [r.row() for r in self.gui.library_view.selectionModel().selectedRows()]
        if not rows or len(rows) == 0:
            d = error_dialog(self.gui, _('Cannot convert'), _('No books selected'))
            d.exec()
            return None
    ...
    def convert_ebooks_to_format(self, book_ids, to_fmt):
        ...
        if to_fmt.lower() not in available_output_formats():
            return error_dialog(
                self.gui,
                _('Cannot convert'),
                _('Conversion to the {} format is not supported').format(to_fmt),
                show=True,
            )
    ...
    def do_convert(self, book_ids, bulk=None, auto_conversion=False, output_fmt=None):
        ...
        if bulk or (bulk is None and len(book_ids) > 1):
            self.__bulk_queue = convert_bulk_ebook(
                ...
                book_ids,
                out_format=output_fmt or prefs['output_format'],
        ...
        if num > 0:
            ...
            self.gui.status_bar.show_message(ngettext('Starting conversion of the book', 'Starting conversion of {} books', num).format(num), 2000)

    def queue_convert_jobs(self, jobs, changed, bad, rows, previous, converted_func, extra_job_args=[], rows_are_ids=False):
        for func, args, desc, fmt, id, temp_files in jobs:
            ...
            if id not in bad:
                job = self.gui.job_manager.run_job(Dispatcher(converted_func), func, args=args, description=desc, core_usage=core_usage)
