...
def render_spine_text_as_pixmap(
    ...
    if pmap := QPixmapCache.find(key):
        return pmap
    ans = QImage(ss.height(), ss.width(), QImage.Format.Format_ARGB32_Premultiplied)
...
class ColorTheme(NamedTuple):
    ...
    @classmethod
    def _from_palette(cls, palette: QPalette) -> ColorTheme:
        return ColorTheme(
...
class RenderCase:
    ...
    def draw_cavity_shadows(self, painter: QPainter, cavity_rect: QRect) -> None:
        ...
        painter.fillRect(
            cavity_rect.right() - side_shadow_width,
            cavity_rect.y(),
            side_shadow_width,
            cavity_rect.height(),
    ...
    def divider_as_pixmap(self, width: int, height: int, divider_color: QColor, corner_radius: int = 0, offset: int = 0) -> QPixmap:
        ...
        ans = QImage(width, height + offset, QImage.Format.Format_ARGB32_Premultiplied)
        ans.fill(Qt.GlobalColor.transparent)
        with QPainter(ans) as painter:
            painter.setRenderHint(QPainter.RenderHint.Antialiasing)
            path = QPainterPath()
            path.addRoundedRect(
                QRectF(0, 0, width, height + offset + corner_radius),
                corner_radius,
                corner_radius,
            )
...
class PixmapWithDominantColor(QPixmap):
    ...
    @staticmethod
    def fromImage(image: QImage | ImageWithDominantColor, flags: Qt.ImageConversionFlag = Qt.ImageConversionFlag.AutoColor) -> PixmapWithDominantColor:
        ans: PixmapWithDominantColor = PixmapWithDominantColor(QPixmap.fromImage(image))
        if not hasattr(image, 'dominant_color'):
            image = ImageWithDominantColor(image)
        assert isinstance(image, ImageWithDominantColor)
        ans.dominant_color = image.dominant_color
        return ans

    def copy_as_same_class(self, x, y, width, height) -> PixmapWithDominantColor:
        ans = super().copy(x, y, width, height)
        q = PixmapWithDominantColor(ans)
        q.dominant_color = self.dominant_color
        return q
...
def draw_pixmap_with_shadow(
    ...
    shadow_color: QColor = QColor(0, 0, 0, 100),
    fill_color: QColor = QColor(Qt.GlobalColor.transparent),
) -> tuple[QPixmap, int]:
    ...
    # Create shadow image
    shadow_image = QImage(total_width, total_height, QImage.Format.Format_ARGB32_Premultiplied)
    ...
    with QPainter(shadow_image) as shadow_painter:
        ...
        if margin:
            ...
            shadow_rect = QRectF(margin + shadow_offset_x, margin + shadow_offset_y, pixmap.width(), pixmap.height())
            ...
            for i in range(shadow_blur, 0, -1):
                ...
                blur_color = QColor(shadow_color.red(), shadow_color.green(), shadow_color.blue(), alpha)
...
def iter_all_groups(fm) -> Iterator[tuple[str, str]]:
    ...
    for k in sorted(cf, key=cf.__getitem__):
        yield k, fm[k]['name']
...
class CaseItem:
    ...
    def add_group_divider(self, group_name: str, lc: LayoutConstraints) -> bool:
        ...
        s = ShelfItem(
            ...
            width=lc.divider_width,
            case_start_y=self.start_y,
            idx=len(self.items),
            case_idx=self.idx,
        )
        self.items.append(s)
    ...
    def add_book(self, book_id: int, width: int, group_name: str, lc: LayoutConstraints, reduce_height_by: int) -> bool:
        ...
        s = ShelfItem(
            start_x=x,
            book_id=book_id,
            reduce_height_by=reduce_height_by,
            width=width,
            group_name=group_name,
            case_start_y=self.start_y,
            idx=len(self.items),
...
def get_grouped_iterator(db: Cache, book_ids_iter: Iterable[int], field_name: str = '') -> Iterator[tuple[str, int | Iterable[int]]]:
    formatter = lambda x: x  # noqa: E731
    fm = db.field_metadata
    sort_key = numeric_sort_key
    get_books_in_group: Callable[[int], set[int]] | Callable[[tuple[int, int]], set[int]] = lambda group: db.books_for_field(field_name, group)  # noqa: E731
    get_field_id_map: Callable[[], dict[int | tuple[int, int], str | tuple[int, int] | int]] = lambda: db.get_id_map(field_name)  # noqa: E731
    ...
    for group in sorted(field_id_map, key=lambda fid: sort_key(field_id_map[fid])):
        books_in_group = (get_books_in_group(group) & all_book_ids) - seen  # type: ignore
        if books_in_group:
            seen |= books_in_group
...
class BookCase(QObject):
    ...
    def invalidate(
        self,
        layout_constraints: LayoutConstraints = LayoutConstraints(),
        model: BooksModel | None = None,
        group_field_name: str = '',
        min_line_height: int = 0,
    ) -> None:
        with self.lock:
            self.current_invalidate_event.set()
            self.current_invalidate_event = Event()
    ...
    def do_layout_in_worker(
        ...
        assert isinstance(num_of_groups, int)
        with self.lock:
            if invalidate.is_set():
                return
        ...
        for group_name, book_ids_in_group in group_iter:
            ...
            assert not isinstance(book_ids_in_group, int)
            for book_id in book_ids_in_group:
                ...
                book_height_reduction = (
                    height_reduction_for_book(book_id, group_height_reduction, height_reduction_is_per_group) if height_reduction_required else 0
                )
                if not current_case_item.add_book(book_id, spine_width, group_name, lc, book_height_reduction):
                    ...
                    if add_group_dividers:
                        if case_end_divider:
                            ...
                            current_case_item.add_group_divider(group_name, lc)
                    if not current_case_item.add_book(book_id, spine_width, group_name, lc, book_height_reduction):
                        raise ValueError(f'Failed to add a single book to a new shelf: {book_id=} {spine_width=} {lc.width=} {lc.max_spine_width=}')
                assert current_case_item.items is not None
    ...
    def end_book_on_shelf_of(self, book_id: int, first: bool = False) -> int:
        if (ci := self.shelf_of_book(book_id)) and ci.items:
...
class ExpandedCover(QObject):  # {{{
    ...
    def shift_items(self) -> None:
        case_item, shelf_item = self.case_item, self.shelf_item
        assert case_item is not None and shelf_item is not None
        self.modified_case_item = case_item.shift_for_expanded_cover(shelf_item, self.layout_constraints, self.size.width())
    ...
    def modify_shelf_layout(self, case_item: CaseItem) -> CaseItem:
        if self.expanded_cover_should_be_displayed and case_item is self.case_item and self.modified_case_item is not None:
    ...
    def draw_expanded_cover(self, painter: QPainter, scroll_y: int, lc: LayoutConstraints, selection_highlight_color: QColor) -> None:
        ...
        pmap, margin = self.cover_renderer.as_pixmap(cover_rect.size(), self.opacity, p)
...
class BookshelfView(MomentumScrollMixin, QAbstractScrollArea):
    ...
    def __init__(self, gui):
        ...
        self.theme: ColorTheme | None = None
        ...
        self.cover_cache = CoverThumbnailCache(
            ...
            ram_limit=800,
            max_size=gprefs['bookshelf_disk_cache_size'],
            thumbnailer=ThumbnailerWithDominantColor(),
    ...
    def render_template(self, column_name: str, template: str, book_id: int) -> str:
        ...
        mi = db.get_proxy_metadata(book_id)
        rslt = mi.formatter.safe_format(template, mi, TEMPLATE_ERROR, mi, column_name=column_name, template_cache=m.bookshelf_template_cache)
        return rslt or ''
    ...
    def render_emblem(self, book_id: int) -> str:
        ...
        for i, (kind, column, rule) in enumerate(self.emblem_rules):
            icon_name, mi = render_emblem(
                book_id,
                rule,
                i,
                m.bookshelf_emblem_cache,
                mi,
                db,
                m.formatter,
                m.bookshelf_template_cache,
                column_name='bookshelf',
            )
            if icon_name:
                return icon_name
        return ''
    ...
    def setModel(self, model: BooksModel | None) -> None:
        ...
        signals = {
            'dataChanged': 'model_data_changed',
            'rowsInserted': 'model_rows_changed',
            'rowsRemoved': 'model_rows_changed',
    ...
    def invalidate(
        self,
        set_of_books_changed: bool = False,
        clear_spine_width_cache: bool = False,
    ) -> None:
        if clear_spine_width_cache:
            self.bookcase.clear_spine_width_cache()
        self.bookcase.invalidate(
            self.layout_constraints,
            model=self.model() if set_of_books_changed else None,
            group_field_name=self.grouping_mode,
            min_line_height=int(self.min_line_height),
        )
    ...
    def draw_emblems(self, painter: QPainter, item: ShelfItem, scroll_y: int) -> tuple[int, int]:
        ...
        if m := self.model():
            ...
            db = db.new_api
            device_connected = get_gui(fail_if_absent=True).device_connected is not None
    ...
    def do_paint(self, painter: QPainter) -> None:
        ...
        painter.drawPixmap(
            QPoint(0, 0),
            self.case_renderer.background_as_pixmap(viewport_rect.width(), viewport_rect.height()),
        )
        ...
        for shelf, has_expanded in shelves:
            ...
            for item in shelf.items:
                ...
                if item is not shelf.expanded_item:
                    ...
                    should_draw_emblems = (
                        not has_expanded
                        or gprefs['bookshelf_hover'] != 'above'
    ...
    def draw_shelf_base(self, painter: QPainter, shelf: CaseItem, scroll_y: int, width: int, instance: int) -> None:
    ...
    def draw_inline_divider(self, painter: QPainter, divider: ShelfItem, scroll_y: int):
        ...
        def draw_rounded_divider(corner_radius: int, offset: int):
            p = self.case_renderer.divider_as_pixmap(rect.width(), rect.height(), theme.divider_background_color, corner_radius, offset)
            painter.drawPixmap(rect.adjusted(0, -offset, 0, 0), p)

        match gprefs['bookshelf_divider_style']:
            case 'block':
                painter.fillRect(rect, theme.divider_background_color)
            case 'gravestone':
                radius = rect.width() // 2
                offset = radius // 4
        ...
        painter.save()
        painter.setFont(font)
        painter.setPen(theme.divider_text_color)
        ...
        if line_rect.width() > 8:
            ...
            color1 = theme.divider_line_color.toRgb()
            color2 = color1.toRgb()
    ...
    def draw_spine(
        ...
        if thumbnail is None:  # not yet rendered
            ...
            assert isinstance(thumbnail, PixmapWithDominantColor)
    ...
    def selection_highlight_color(self, is_selected: bool, is_current: bool) -> QColor:
        ...
        if is_current and is_selected:
            return theme.current_selected_color
        if is_current:
            return theme.current_color
        if is_selected:
            return theme.selected_color
        return QColor()
    ...
    def draw_spine_title(
        ...
        nfl, nsl, font, fm, was_wrapped = self.get_text_metrics(
            first_line, second_line, first_rect.transposed().size(), allow_wrap=True, outline_width=self.outline_width
        )
        if not nfl and not nsl:  # two lines dont fit
            second_line = ''
            first_rect = QRect(rect.left(), first_rect.top(), rect.width(), first_rect.height())
            nfl, nsl, font, fm, _ = self.get_text_metrics(first_line, second_line, first_rect.transposed().size(), outline_width=self.outline_width)
        elif was_wrapped:
        ...
        def draw_text(text: str, rect: QRect, alignment: Qt.AlignmentFlag) -> None:
            pixmap = render_spine_text_as_pixmap(
                ...
                font,
                fm,
                rect.transposed().size(),
                alignment,
                gprefs['bookshelf_up_to_down'],
                self.outline_width,
    ...
    def draw_spine_cover(self, painter: QPainter, rect: QRect, thumbnail: PixmapWithDominantColor) -> None:
        match gprefs['bookshelf_thumbnail']:
            ...
            # Adjust size
            case 'crops':
                thumbnail = thumbnail.copy_as_same_class(0, 0, rect.width(), thumbnail.height())
            case 'edge':
                width = round(max(10, rect.width() * 0.2))
                thumbnail = thumbnail.copy_as_same_class(0, 0, width, thumbnail.height())
    ...
    def load_hover_cover(self, si: ShelfItem) -> tuple[PixmapWithDominantColor, QSize]:
        ...
        if cover_img is None or cover_img.isNull():
            ...
            if isinstance(tb, PixmapWithDominantColor) and tb.dominant_color.isValid():
    ...
    def get_contrasting_text_color(self, background_color: QColor) -> tuple[QColor, QColor]:
        ...
        if not background_color or not background_color.isValid():
            return theme.text_color_for_light_background, theme.outline_color_for_light_background
        if contrast_ratio(background_color, theme.text_color_for_dark_background) > contrast_ratio(background_color, theme.text_color_for_light_background):
            return theme.text_color_for_dark_background, theme.outline_color_for_dark_background
        return theme.text_color_for_light_background, theme.outline_color_for_light_background
    ...
    def handle_mouse_move_event(self, ev: QMouseEvent):
        ...
        if ev.buttons() & Qt.MouseButton.LeftButton:
            handle_selection_drag(
                self,
                self.indexAt(ev.pos()),
                self.click_start_data,
                self.bookcase.visual_row_cmp,
