...
static inline void *
guarded_malloc(size_t x) {
    return x ? malloc(x) : NULL;
}

enum { OP_EQUAL = 0, OP_INSERT, OP_DELETE, OP_REPLACE };
...
typedef struct {
    PyObject_HEAD Py_ssize_t asize;
    Py_ssize_t bsize;
    struct line *a;
    struct line *b;
...
compare_lines(struct line *a, struct line *b) {
    return ((a->hash != b->hash) || PyObject_RichCompareBool(a->data, b->data, Py_NE));
}


static inline int
find_equivalence_class(struct bucket *hashtable, Py_ssize_t hsize, struct line *lines, struct line *ref_lines, Py_ssize_t i) {
    Py_ssize_t j;
    for (j = lines[i].hash & hsize; hashtable[j].b_head != SENTINEL; j = (j + 1) & hsize) {
        if (!compare_lines(lines + i, ref_lines + hashtable[j].b_head)) { break; }
    }
    return j;
}
...
static int
equate_lines(struct hashtable *result, struct line *lines_a, struct line *lines_b, Py_ssize_t asize, Py_ssize_t bsize) {
    ...
    for (i = asize - 1; i >= 0; i--) {
        ...
        if (hashtable[j].b_head == SENTINEL) continue;
...
unique_lcs(
    ...
    for (i = 0; i < bsize; i++) backpointers[i] = SENTINEL;

    if (hashtable->last_a_pos == -1 || hashtable->last_a_pos > alo)
        for (i = 0; i < hashtable->size; i++) h[i].a_pos = h[i].a_head;
    hashtable->last_a_pos = alo;

    if (hashtable->last_b_pos == -1 || hashtable->last_b_pos > blo)
        for (i = 0; i < hashtable->size; i++) h[i].b_pos = h[i].b_head;
    ...
    for (bpos = blo; bpos < bhi; bpos++) {
        ...
        /* no lines in a or b  */
        if (h[equiv].a_count == 0 || h[equiv].b_count == 0) continue;
        ...
        */
        ...
        if (stacksize && stacks[stacksize - 1] < norm_apos) k = stacksize;
        /* as an optimization, check if the next line comes right after
           the previous line, because usually it does */
        else if (stacksize && (stacks[k] < norm_apos) && (k == stacksize - 1 || stacks[k + 1] > norm_apos)) k += 1;
        else k = bisect_left(stacks, norm_apos, 0, stacksize);

        if (k > 0) backpointers[norm_bpos] = lasts[k - 1];
...
add_matching_line(struct matching_blocks *answer, Py_ssize_t a, Py_ssize_t b) {
    ...
    if ((last_index >= 0) && (a == answer->matches[last_index].a + answer->matches[last_index].len) &&
        (b == answer->matches[last_index].b + answer->matches[last_index].len)) {
...
recurse_matches(
    ...
    for (i = lcs_size - 1; i >= 0; i--) {
        ...
        if (last_a_pos + 1 != apos || last_b_pos + 1 != bpos) {
            res = recurse_matches(answer, hashtable, backpointers, a, b, last_a_pos + 1, last_b_pos + 1, apos, bpos, maxrecursion - 1);
            if (!res) goto error;
        }
        last_a_pos = apos;
        last_b_pos = bpos;
    ...
    if (new > 0) {
        res = recurse_matches(answer, hashtable, backpointers, a, b, last_a_pos + 1, last_b_pos + 1, ahi, bhi, maxrecursion - 1);
        if (!res) goto error;
    }
    ...
    /* find matching lines at the very beginning */
        ...
        while (alo < ahi && blo < bhi && a[alo].equiv == b[blo].equiv) add_matching_line(answer, alo++, blo++);
        res = recurse_matches(answer, hashtable, backpointers, a, b, alo, blo, ahi, bhi, maxrecursion - 1);
        if (!res) goto error;
    }
    ...
    /* find matching lines at the very end */
        ...
        res = recurse_matches(answer, hashtable, backpointers, a, b, last_a_pos + 1, last_b_pos + 1, nahi, nbhi, maxrecursion - 1);
        if (!res) goto error;
        for (i = 0; i < ahi - nahi; i++) add_matching_line(answer, nahi + i, nbhi + i);
    }
...
py_unique_lcs(PyObject *self, PyObject *args) {
    ...
    for (i = 0; i < nmatches; i++) {
        ...
        if (PyList_SetItem(res, i, item) != 0) goto error;
    }
...
py_recurse_matches(PyObject *self, PyObject *args) {
...
#if PY_VERSION_HEX < 0x02050000
    if (!PyArg_ParseTuple(args, "OOiiiiOi", &aseq, &bseq, &alo, &blo, &ahi, &bhi, &answer, &maxrecursion))
#else
    if (!PyArg_ParseTuple(args, "OOnnnnOi", &aseq, &bseq, &alo, &blo, &ahi, &bhi, &answer, &maxrecursion))
#endif
        return NULL;
    ...
    if (bsize > 0) {
        ...
        if (matches.matches == NULL) goto error;
    ...
    res = recurse_matches(&matches, &hashtable, backpointers, a, b, alo, blo, ahi, bhi, maxrecursion);
    if (!res) goto error;

    for (i = 0; i < matches.count; i++) {
        for (j = 0; j < matches.matches[i].len; j++) {
#if PY_VERSION_HEX < 0x02050000
            item = Py_BuildValue("ii", matches.matches[i].a + j, matches.matches[i].b + j);
#else
            item = Py_BuildValue("nn", matches.matches[i].a + j, matches.matches[i].b + j);
#endif
            if (item == NULL) goto error;
            if (PyList_Append(answer, item) != 0) goto error;
        }
    }
...
static char PatienceSequenceMatcher_get_matching_blocks_doc[] = "Return list of triples describing matching subsequences.\n"
                                                                "\n"
...
PatienceSequenceMatcher_get_matching_blocks(PatienceSequenceMatcher *self) {
    ...
    if (self->bsize > 0) {
        ...
        if (matches.matches == NULL) return PyErr_NoMemory();
    } else matches.matches = NULL;

    res = recurse_matches(&matches, &self->hashtable, self->backpointers, self->a, self->b, 0, 0, self->asize, self->bsize, 10);
    if (!res) {
        free(matches.matches);
    ...
    for (i = 0; i < matches.count; i++) {
#if PY_VERSION_HEX < 0x02050000
        item = Py_BuildValue("iii", matches.matches[i].a, matches.matches[i].b, matches.matches[i].len);
#else
        item = Py_BuildValue("nnn", matches.matches[i].a, matches.matches[i].b, matches.matches[i].len);
#endif
        if (item == NULL) goto error;
        if (PyList_SetItem(answer, i, item) != 0) goto error;
    }
#if PY_VERSION_HEX < 0x02050000
...
static char PatienceSequenceMatcher_get_opcodes_doc[] = "Return list of 5-tuples describing how to turn a into b.\n"
                                                        "\n"
...
PatienceSequenceMatcher_get_opcodes(PatienceSequenceMatcher *self) {
    ...
    for (k = 0; k < matches.count; k++) {
        ...
        tag = -1;
        if (i < ai && j < bj) tag = OP_REPLACE;
        else if (i < ai) tag = OP_DELETE;
        else if (j < bj) tag = OP_INSERT;
...
static char PatienceSequenceMatcher_get_grouped_opcodes_doc[] = "Isolate change clusters by eliminating ranges with no changes.\n"
                                                                "\n"
...
static PyObject *
PatienceSequenceMatcher_get_grouped_opcodes(PatienceSequenceMatcher *self, PyObject *args) {
    ...
    if (codes[ncodes - 1].tag == OP_EQUAL) {
        codes[ncodes - 1].i2 = MIN(codes[ncodes - 1].i2, codes[ncodes - 1].i1 + n);
        codes[ncodes - 1].j2 = MIN(codes[ncodes - 1].j2, codes[ncodes - 1].j1 + n);
    }

    group = NULL;

    answer = PyList_New(0);
    if (answer == NULL) goto error;
...
#if PY_VERSION_HEX < 0x02050000
            item = Py_BuildValue("siiii", opcode_names[tag], i1, MIN(i2, i1 + n), j1, MIN(j2, j1 + n));
#else
            item = Py_BuildValue("snnnn", opcode_names[tag], i1, MIN(i2, i1 + n), j1, MIN(j2, j1 + n));
#endif
            if (item == NULL) goto error;
            if (PyList_Append(group, item) != 0) goto error;
            if (PyList_Append(answer, group) != 0) goto error;
            group = PyList_New(0);
            if (group == NULL) goto error;
            i1 = MAX(i1, i2 - n);
            j1 = MAX(j1, j2 - n);
...
static PyMethodDef PatienceSequenceMatcher_methods[] = {
    {"get_matching_blocks", (PyCFunction)PatienceSequenceMatcher_get_matching_blocks, METH_NOARGS, PatienceSequenceMatcher_get_matching_blocks_doc},
    {"get_opcodes", (PyCFunction)PatienceSequenceMatcher_get_opcodes, METH_NOARGS, PatienceSequenceMatcher_get_opcodes_doc},
    {"get_grouped_opcodes", (PyCFunction)PatienceSequenceMatcher_get_grouped_opcodes, METH_VARARGS, PatienceSequenceMatcher_get_grouped_opcodes_doc},
    {NULL}};


static char PatienceSequenceMatcher_doc[] = "C implementation of PatienceSequenceMatcher";
...
static PyMethodDef _patiencediff_c_methods[] = {
    {"unique_lcs_c", py_unique_lcs, METH_VARARGS}, {"recurse_matches_c", py_recurse_matches, METH_VARARGS}, {NULL, NULL, 0, NULL}};

static int
exec_module(PyObject *mod) {
    if (PyType_Ready(&PatienceSequenceMatcherType) < 0) return -1;
    Py_INCREF(&PatienceSequenceMatcherType);
    PyModule_AddObject(mod, "PatienceSequenceMatcher_c", (PyObject *)&PatienceSequenceMatcherType);
    return 0;
}
...
PyInit__patiencediff_c(void) { return PyModuleDef_Init(&module_def); }
