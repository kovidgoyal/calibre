...
disable_window_tabbing(void) {
    if ([NSWindow respondsToSelector:@selector(allowsAutomaticWindowTabbing)]) NSWindow.allowsAutomaticWindowTabbing = NO;
}
...
- (void)userNotificationCenter:(UNUserNotificationCenter *)center
    ...
    if (notification_activated_callback) {
        ...
        PyObject *ret = PyObject_CallFunction(notification_activated_callback, "z", identifier ? [identifier UTF8String] : NULL);
...
schedule_notification(const char *identifier, const char *title, const char *body, const char *subtitle, bool use_sound) {
    ...
    [center addNotificationRequest:request
             withCompletionHandler:^(NSError *_Nullable error) {
               if (error != nil) { fprintf(stderr, "Failed to show notification: %s\n", [[error localizedDescription] UTF8String]); }
             }];
    [content release];
}
...
send_notification(PyObject *self, PyObject *args) {
    ...
    [center requestAuthorizationWithOptions:(UNAuthorizationOptionAlert | UNAuthorizationOptionSound | UNAuthorizationOptionBadge)
                          completionHandler:^(BOOL granted, NSError *_Nullable error) {
                            ...
                            dispatch_async(dispatch_get_main_queue(), ^{ drain_pending_notifications(granted); });
                          }];
    Py_RETURN_NONE;
}
// }}}
...
locale_names(PyObject *self, PyObject *args) {
    ...
    for (Py_ssize_t i = 0; i < PyTuple_GET_SIZE(ans); i++) {
        ...
        if (!PyUnicode_Check(x)) {
            PyErr_SetString(PyExc_TypeError, "language codes must be unicode");
            Py_CLEAR(ans);
            return NULL;
        }
...
create_io_pm_assertion(PyObject *self, PyObject *args) {
    ...
    if (s == nil) {
        PyErr_SetString(PyExc_TypeError, "type argument must be a valid UTF-8 string");
        return NULL;
    }
    CFStringRef r = CFStringCreateWithCString(NULL, reason, kCFStringEncodingUTF8);
    if (r == nil) {
        CFRelease(s);
        PyErr_SetString(PyExc_TypeError, "reason argument must be a valid UTF-8 string");
        return NULL;
    }
...
static PyMethodDef module_methods[] = {
    {"close_finder_window",
     (PyCFunction)close_finder_window,
     METH_VARARGS,
     "close_finder_window(name) -> None -- Close a Finder window whose title matches name."},
...
#define A(which) \
    if (PyModule_AddStringConstant(module, #which, [(__bridge NSString *)which UTF8String]) == -1) return -1;
