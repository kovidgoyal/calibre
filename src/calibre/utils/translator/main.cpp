...
add_fallback(PyObject *self_, PyObject *pn) {
    Translator *self = (Translator *)self_;
    if (Py_TYPE(pn) != &Translator_Type) {
        PyErr_SetString(PyExc_TypeError, "other must be a translator instance");
        return NULL;
    }
...
install(PyObject *self, PyObject *args, PyObject *kwargs) {
    ...
    if (names != NULL && names != Py_None && !PySequence_Check(names)) {
        PyErr_SetString(PyExc_TypeError, "names must be a sequence");
        return NULL;
    }
...
#define S(name, method_name)                                          \
    {                                                                 \
        ...
        int ret = PyDict_SetItemString(builtins_dict, name, method);  \
        Py_DECREF(method);                                            \
        if (ret != 0) {                                               \
            Py_DECREF(builtins_dict);                                 \
            Py_DECREF(builtins_module);                               \
...
static bool
has_data(Translator *self) {
    return self->parser.isLoaded() || PyList_GET_SIZE(self->fallbacks) != 0;
}
...
gettext(PyObject *s, PyObject *msg) {
    ...
    if (!PyUnicode_Check(msg)) {
        PyErr_SetString(PyExc_TypeError, "message must be a string");
        return NULL;
    }
    Py_ssize_t sz;
...
npgettext(PyObject *s, PyObject *args) {
    Translator *self = (Translator *)s;
    const char *context, *singular, *plural;
    unsigned long n;
    Py_ssize_t cl, sl, pl;
...
qt_translate(const char *context, const char *text) {
    ...
    if (context && context[0]) {
        buffer.append(context).push_back('\0');
        buffer.append(text);
        key = buffer.c_str();
    }
...
PyTypeObject Translator_Type = {
    .ob_base = PyVarObject_HEAD_INIT(NULL, 0).tp_name = "translator.Translator",
