...
'''
    ...
    def evaluate(self, formatter, kwargs, mi, locals, *args):
        ...
        if qs:
            qs = '?' + qs
        return f"{scheme}://{authority}{'/' if authority else ''}{path.removeprefix('/')}{qs}"
    ...
    def evaluate(self, formatter, kwargs, mi, locals, *args):
        ...
        for i in range(0, len(args), 3):
            if (f := args[i + 2]) not in ('0', '1', '2'):
                raise ValueError(_('In {} the third argument of a group must be 0, 1, or 2, not {}').format('query_string', f))
...
'''
    ...
    def evaluate(self, formatter, kwargs, mi, locals, *args):
        ...
        if use_plus not in ('0', '1'):
            raise ValueError(_('In {} the second argument must be 0, or 1, not {}').format('quote_for_url', use_plus))
