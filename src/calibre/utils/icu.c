...
static PyObject *
uchar_to_unicode(const UChar *src, int32_t len) {
    wchar_t *buf = NULL;
    PyObject *ans = NULL;
...
icu_Collator_display_name(icu_Collator *self, void *closure) {
    ...
    if (loc == NULL) {
        PyErr_SetString(PyExc_Exception, "Failed to get actual locale");
        return NULL;
    }
...
icu_Collator_strcmp(icu_Collator *self, PyObject *const *args, Py_ssize_t nargs) {
    ...
    if (nargs != 2) {
        PyErr_SetString(PyExc_TypeError, "strcmp takes exactly 2 arguments");
        return NULL;
    }
    a_ = args[0];
    b_ = args[1];
...
icu_Collator_find(icu_Collator *self, PyObject *const *args, Py_ssize_t nargs) {
    ...
    if (nargs < 2 || nargs > 3) {
        PyErr_SetString(PyExc_TypeError, "find requires 2 or 3 arguments");
        return NULL;
    }
    a_ = args[0];
    b_ = args[1];
    if (!PyUnicode_Check(a_) || !PyUnicode_Check(b_)) {
        PyErr_SetString(PyExc_TypeError, "pattern and source must be unicode strings");
        return NULL;
    }
...
icu_Collator_find_all(icu_Collator *self, PyObject *const *args, Py_ssize_t nargs) {
    ...
    if (nargs < 3 || nargs > 4) {
        PyErr_SetString(PyExc_TypeError, "find_all requires 3 or 4 arguments");
        return NULL;
    }
    a_ = args[0];
    b_ = args[1];
...
icu_Collator_contains(icu_Collator *self, PyObject *const *args, Py_ssize_t nargs) {
    ...
    if (nargs != 2) {
        PyErr_SetString(PyExc_TypeError, "contains takes exactly 2 arguments");
        return NULL;
    }
    a_ = args[0];
    b_ = args[1];
...
icu_Collator_startswith(icu_Collator *self, PyObject *const *args, Py_ssize_t nargs) {
    ...
    unsigned offset = 0;
    if (nargs < 2 || nargs > 3) {
        PyErr_SetString(PyExc_TypeError, "startswith requires 2 or 3 arguments");
        return NULL;
    }
    a_ = args[0];
    b_ = args[1];
...
icu_Collator_get_attribute(icu_Collator *self, PyObject *const *args, Py_ssize_t nargs) {
    if (nargs != 1) {
        PyErr_SetString(PyExc_TypeError, "get_attribute takes exactly 1 argument");
        return NULL;
    }
...
icu_Collator_set_attribute(icu_Collator *self, PyObject *const *args, Py_ssize_t nargs) {
    if (nargs != 2) {
        PyErr_SetString(PyExc_TypeError, "set_attribute takes exactly 2 arguments");
        return NULL;
    }
    int k = (int)PyLong_AsLong(args[0]);
    if (PyErr_Occurred()) return NULL;
...
static PyObject *icu_Collator_clone(icu_Collator *self, PyObject *args);

static PyMethodDef icu_Collator_methods[] = {
    {"sort_key",
     (PyCFunction)icu_Collator_sort_key,
     METH_O,
     "sort_key(unicode object) -> Return a sort key for the given object as a bytestring. The idea is that these bytestring will sort using the builtin cmp "
     "function, just like the original unicode strings would sort in the current locale with ICU."},

    {"get_attribute",
     (PyCFunction)(void (*)(void))icu_Collator_get_attribute,
     METH_FASTCALL,
     "get_attribute(key) -> get the specified attribute on this collator."},

    {"set_attribute",
     (PyCFunction)(void (*)(void))icu_Collator_set_attribute,
     METH_FASTCALL,
     "set_attribute(key, val) -> set the specified attribute on this collator."},

    {"strcmp",
     (PyCFunction)(void (*)(void))icu_Collator_strcmp,
     METH_FASTCALL,
     "strcmp(unicode object, unicode object) -> strcmp(a, b) <=> cmp(sorty_key(a), sort_key(b)), but faster."},

    {"find_all",
     (PyCFunction)(void (*)(void))icu_Collator_find_all,
     METH_FASTCALL,
    ...
    {"find",
     (PyCFunction)(void (*)(void))icu_Collator_find,
     METH_FASTCALL,
    ...
    {"contains",
     (PyCFunction)(void (*)(void))icu_Collator_contains,
     METH_FASTCALL,
     "contains(pattern, source) -> return True iff the pattern was found in the source."},

    {"contractions", (PyCFunction)icu_Collator_contractions, METH_NOARGS, "contractions() -> returns the contractions defined for this collator."},

    {"clone", (PyCFunction)icu_Collator_clone, METH_NOARGS, "clone() -> returns a clone of this collator."},

    {"startswith",
     (PyCFunction)(void (*)(void))icu_Collator_startswith,
     METH_FASTCALL,
    ...
    {"collation_order",
     (PyCFunction)icu_Collator_collation_order,
     METH_O,
     "collation_order(string) -> returns (order, length) where order is an integer that gives the position of string in a list. length gives the number of "
     "characters used for order."},

    {NULL} /* Sentinel */
};

static PyGetSetDef icu_Collator_getsetters[] = {
    {(char *)"actual_locale", (getter)icu_Collator_actual_locale, NULL, (char *)"Actual locale used by this collator.", NULL},

    {(char *)"capsule", (getter)icu_Collator_capsule, NULL, (char *)"A capsule enclosing the pointer to the ICU collator struct", NULL},

    {(char *)"display_name",
     (getter)icu_Collator_display_name,
     NULL,
     (char *)"Display name of this collator in English. The name reflects the actual data source used.",
     NULL},

    {(char *)"strength", (getter)icu_Collator_get_strength, (setter)icu_Collator_set_strength, (char *)"The strength of this collator.", NULL},

    {(char *)"upper_first",
     (getter)icu_Collator_get_upper_first,
     (setter)icu_Collator_set_upper_first,
     (char *)"Whether this collator should always put upper case letters before lower case. Values are: None - means use the tertiary strength of the letters. "
             "True - Always sort upper case before lower case. False - Always sort lower case before upper case.",
     NULL},

    {(char *)"numeric",
     (getter)icu_Collator_get_numeric,
     (setter)icu_Collator_set_numeric,
     (char *)"If True the collator sorts contiguous digits as numbers rather than strings, so 2 will sort before 10.",
     NULL},

    {(char *)"max_variable",
     (getter)icu_Collator_get_max_variable,
     (setter)icu_Collator_set_max_variable,
...
icu_Transliterator_new(PyTypeObject *type, PyObject *args, PyObject *kwds) {
    ...
    if (t == NULL || U_FAILURE(status)) {
        ...
        PyErr_Format(
            PyExc_ValueError,
            "Failed to compile Transliterator with error: %s line: %d offset: %d pre: %U post: %U",
            u_errorName(status),
            pe.line,
            pe.offset,
            pre,
...
static int32_t
replaceable_length(const UReplaceable *rep) {
    const Replaceable *x = (const Replaceable *)rep;
    return x->sz;
}

static UChar
replaceable_charAt(const UReplaceable *rep, int32_t offset) {
    const Replaceable *x = (const Replaceable *)rep;
...
static UChar32
replaceable_char32At(const UReplaceable *rep, int32_t offset) {
    const Replaceable *x = (const Replaceable *)rep;
    if (offset >= x->sz || offset < 0) return 0xffff;
    UChar32 c;
...
static void
replaceable_replace(UReplaceable *rep, int32_t start, int32_t limit, const UChar *text, int32_t repl_len) {
    Replaceable *x = (Replaceable *)rep;
    /* printf("start replace: start=%d limit=%d x->sz: %d text=%s repl_len=%d\n", start, limit, x->sz, PyUnicode_AsUTF8(icu_to_python(text, repl_len)),
     * repl_len); */
...
static void
replaceable_copy(UReplaceable *rep, int32_t start, int32_t limit, int32_t dest) {
    Replaceable *x = (Replaceable *)rep;
...
static void
replaceable_extract(UReplaceable *rep, int32_t start, int32_t limit, UChar *dst) {
    Replaceable *x = (Replaceable *)rep;
...
static PyMethodDef icu_Transliterator_methods[] = {
    {"transliterate", (PyCFunction)icu_Transliterator_transliterate, METH_O, "transliterate(text) -> Run the transliterator on the specified text"},
...
icu_BreakIterator_dealloc(icu_BreakIterator *self) {
    ...
    self->break_iterator = NULL;
    self->text = NULL;
    self->extra_word_break_chars = NULL;
    Py_TYPE(self)->tp_free((PyObject *)self);
}
...
icu_BreakIterator_new(PyTypeObject *type, PyObject *args, PyObject *kwds) {
    ...
    self->text = NULL;
    self->text_len = 0;
    self->type = break_iterator_type;
    self->extra_word_break_chars = NULL;
    self->num_extra_word_break_chars = 0;
    self->hyphen_is_extra_break = 0;

    /* Parse optional extra break characters (only meaningful for UBRK_WORD). */
    if (extra_chars_obj != NULL && extra_chars_obj != Py_None && break_iterator_type == UBRK_WORD) {
        int32_t extra_sz = 0;
        ...
        if (count > 0) {
            ...
            int32_t i = 0, j = 0;
            while (i < extra_sz) {
                U16_NEXT(extra_buf, i, extra_sz, chars[j]);
                j++;
            }
            free(extra_buf);
            ...
            for (int32_t k = 0; k < count; k++) {
                if (IS_HYPHEN_CHAR(chars[k])) {
                    self->hyphen_is_extra_break = 1;
                    break;
                }
            }
        } else {
...
icu_BreakIterator_index(icu_BreakIterator *self, PyObject *token) {
    ...
    while (p != UBRK_DONE) {
        word_start = p;
        p = ubrk_next(self->break_iterator);
        if (self->type == UBRK_WORD && ubrk_getRuleStatus(self->break_iterator) == UBRK_WORD_NONE) continue; // We are not at the start of a word

        if (self->text_len >= word_start + sz && memcmp(self->text + word_start, needle, sz * sizeof(UChar)) == 0) {
            if (word_start > 0 &&
                ((leading_hyphen && !IS_HYPHEN_CHAR(self->text[word_start - 1])) || (!leading_hyphen && IS_HYPHEN_CHAR(self->text[word_start - 1]))))
                continue;
            if (!trailing_hyphen && IS_HYPHEN_CHAR(self->text[word_start + sz])) continue;

            if (p == UBRK_DONE || self->text_len <= word_start + sz) {
                ans = word_start;
                break;
            }
            ...
            if (
                ...
                (!trailing_hyphen || ubrk_following(self->break_iterator, word_start + sz) == UBRK_DONE ||
                 ubrk_getRuleStatus(self->break_iterator) == UBRK_WORD_NONE)) {
                ans = word_start;
                break;
            }
...
find_extra_word_break(const icu_BreakIterator *bi, int32_t start, int32_t end, int32_t *char_len_out) {
    int32_t i = start;
...
static int
break_iter_state_next(icu_BreakIterator *bi, BreakIterState *state, int32_t *pos_out, int32_t *sz_out) {
    ...
    for (;;) {
        ...
        if (state->extra_break_active) {
            if (state->extra_break_seg_start >= state->extra_break_seg_end) {
                ...
                int32_t char_len;
                int32_t q = find_extra_word_break(bi, state->extra_break_seg_start, state->extra_break_seg_end, &char_len);
        ...
        if (bi->type == UBRK_WORD && ubrk_getRuleStatus(bi->break_iterator) == UBRK_WORD_NONE) continue; /* skip non-word runs (spaces, punctuation, …) */
...
icu_BreakIterator_split2(icu_BreakIterator *self, PyObject *args) {
    ...
    while (break_iter_state_next(self, &state, &pos, &sz)) {
        ...
        if (item == NULL || PyList_Append(ans, item) != 0) {
            Py_XDECREF(item);
            Py_DECREF(ans);
            return NULL;
        }
...
typedef struct {
    PyObject_HEAD icu_BreakIterator *parent; /* strong reference */
...
icu_BreakIteratorIter_iternext(icu_BreakIteratorIterObject *self) {
    ...
    if (self->parent->counter != self->counter_at_creation) {
        PyErr_SetString(PyExc_RuntimeError, "BreakIterator was modified while iterating over it");
        return NULL;
    }
    if (!break_iter_state_next(self->parent, &self->state, &pos, &sz)) return NULL; /* StopIteration */
...
static PyMethodDef icu_BreakIterator_methods[] = {
    {"set_text", (PyCFunction)icu_BreakIterator_set_text, METH_O, "set_text(unicode object) -> Set the text this iterator will operate on"},

    {"split2",
     (PyCFunction)icu_BreakIterator_split2,
     METH_NOARGS,
     "split2() -> Split the current text into tokens, returning a list of 2-tuples of the form (position of token, length of token). The numbers are suitable "
     "for indexing python strings regardless of narrow/wide builds."},

    {"iter_breaks",
     (PyCFunction)icu_BreakIterator_iter_breaks,
     METH_NOARGS,
     "iter_breaks() -> Split the current text into tokens, returning an iterator that yields 2-tuples of the form (position of token, length of token). The "
     "numbers are suitable for indexing python strings regardless of narrow/wide builds."},

    {"iter_positions",
     (PyCFunction)icu_BreakIterator_iter_positions,
     METH_NOARGS,
     "iter_positions() -> Split the current text into tokens, returning an iterator that yields the position of each token as an integer. The numbers are "
     "suitable for indexing python strings regardless of narrow/wide builds."},

    {"count_words",
     (PyCFunction)icu_BreakIterator_count_words,
     METH_NOARGS,
     "count_words() -> Split the current text into tokens as in split2() and count the number of tokens."},

    {"index",
     (PyCFunction)icu_BreakIterator_index,
     METH_O,
     "index(token) -> Find the index of the first match for token. Useful to find, for example, words that could also be a part of a larger word. For example, "
     "index('i') in 'string i' will be 7 not 3. Returns -1 if not found."},
...
static PyObject *
icu_change_case(PyObject *self, PyObject *const *args, Py_ssize_t nargs) {
    const char *locale = NULL;
    ...
    if (nargs != 3) {
        PyErr_SetString(PyExc_TypeError, "change_case takes exactly 3 arguments");
        return NULL;
    }
    input = args[0];
    ...
    if (args[2] == Py_None) locale = NULL;
        ...
        locale = PyUnicode_AsUTF8(args[2]);
        if (!locale) return NULL;
    } else if (PyBytes_Check(args[2])) {
        locale = PyBytes_AS_STRING(args[2]);
    } else {
        PyErr_SetString(PyExc_TypeError, "locale must be a string or None");
        return NULL;
    }
    if (locale == NULL) {
        PyErr_SetString(
            PyExc_NotImplementedError,
            "You must specify a locale"); // We deliberately use NotImplementedError so that this error can be unambiguously identified
        return NULL;
    }
    ...
    switch (which) {
        case TITLE_CASE: sz = u_strToTitle(output_buf, 3 * sz, input_buf, sz, NULL, locale, &status); break;
        case UPPER_CASE: sz = u_strToUpper(output_buf, 3 * sz, input_buf, sz, locale, &status); break;
        default: sz = u_strToLower(output_buf, 3 * sz, input_buf, sz, locale, &status);
    }
    if (U_FAILURE(status)) {
        PyErr_SetString(PyExc_ValueError, u_errorName(status));
        goto end;
...
static PyObject *
icu_swap_case(PyObject *self, PyObject *input) {
...
icu_set_filesystem_encoding(PyObject *self, PyObject *const *args, Py_ssize_t nargs) {
    const char *encoding;
    if (nargs != 1) {
        PyErr_SetString(PyExc_TypeError, "set_filesystem_encoding takes exactly 1 argument");
        return NULL;
    }
...
icu_get_available_transliterators(PyObject *self, PyObject *args) {
    ...
    if (i == NULL || U_FAILURE(status)) {
        Py_DECREF(ans);
        PyErr_SetString(PyExc_RuntimeError, "Failed to create enumerator");
        return NULL;
    }
...
icu_character_name(PyObject *self, PyObject *const *args, Py_ssize_t nargs) {
    ...
    if (nargs < 1 || nargs > 2) {
        PyErr_SetString(PyExc_TypeError, "character_name takes 1 or 2 arguments");
        return NULL;
    }
    input = args[0];
    ...
    if (U_FAILURE(status)) {
        PyErr_SetString(PyExc_ValueError, "Failed to get name for code");
        goto end;
    }
...
icu_character_name_from_code(PyObject *self, PyObject *const *args, Py_ssize_t nargs) {
    ...
    if (nargs < 1 || nargs > 2) {
        PyErr_SetString(PyExc_TypeError, "character_name_from_code takes 1 or 2 arguments");
        return NULL;
    }
    unsigned long code_ul = PyLong_AsUnsignedLong(args[0]);
    if (PyErr_Occurred()) return NULL;
    if (code_ul > 0x10FFFF) {
        PyErr_SetString(PyExc_ValueError, "code point out of range(0x110000)");
        return NULL;
    }
...
icu_chr(PyObject *self, PyObject *const *args, Py_ssize_t nargs) {
    ...
    if (nargs != 1) {
        PyErr_SetString(PyExc_TypeError, "chr takes exactly 1 argument");
        return NULL;
    }
    unsigned long code_ul = PyLong_AsUnsignedLong(args[0]);
    if (PyErr_Occurred()) return NULL;
    if (code_ul > 0x10FFFF) {
        PyErr_SetString(PyExc_ValueError, "arg not in range(0x110000)");
        return NULL;
    }
    code = (UChar32)code_ul;
...
icu_normalize(PyObject *self, PyObject *const *args, Py_ssize_t nargs) {
    ...
    if (nargs != 2) {
        PyErr_SetString(PyExc_TypeError, "normalize takes exactly 2 arguments");
        return NULL;
    }
    ...
    switch (mode) {
        case NFC: n = unorm2_getNFCInstance(&status); break;
        case NFKC: n = unorm2_getNFKCInstance(&status); break;
        case NFD: n = unorm2_getNFDInstance(&status); break;
        case NFKD: n = unorm2_getNFKDInstance(&status); break;
    }
    if (U_FAILURE(status)) {
        PyErr_SetString(PyExc_ValueError, u_errorName(status));
        goto end;
...
icu_word_prefix_find(PyObject *self, PyObject *const *args, Py_ssize_t nargs) {
    ...
    if (nargs != 4) {
        PyErr_SetString(PyExc_TypeError, "word_prefix_find takes exactly 4 arguments");
        return NULL;
    }
    collator_obj = args[0];
    it_obj = args[1];
    ...
    while (break_iter_state_next(it, &state, &pos, &sz)) {
        ...
        if (it->text_len - utf16_start >= prefix_sz && ucol_equal(collator->collator, it->text + utf16_start, prefix_sz, prefix_icu, prefix_sz)) {
            ans = (long)pos;
            break;
        }
    }
...
static PyMethodDef icu_methods[] = {
    {"bidi_reorder",
     ...
     METH_FASTCALL,
     "bidi_reorder(text, to_logical=True, want_map=False) -> Reorder text between visual and logical order running the Unicode bidirectional algorithm, "
     "mirroring characters such as brackets as needed. Returns the reordered text, or (text, index_map) if want_map is True, where index_map[i] is the index "
     "in the input of the character at index i of the output (-1 if there is none)."},
    ...
    {"swap_case", icu_swap_case, METH_O, "swap_case(unicode object) -> swaps the case using the simple, locale independent unicode algorithm"},

    {"set_default_encoding",
     icu_set_default_encoding,
     METH_NOARGS,
    ...
    {"ord_string", icu_ord_string, METH_O, "ord_string(code) -> Convert a python unicode string to a tuple of unicode codepoints."},

    {"normalize",
     (PyCFunction)(void (*)(void))icu_normalize,
     METH_FASTCALL,
     "normalize(mode, unicode_text) -> Return a python unicode string which is normalized in the specified mode."},

    {"roundtrip", icu_roundtrip, METH_O, "roundtrip(string) -> Roundtrip a unicode object from python to ICU back to python (useful for testing)"},

    {"available_locales_for_break_iterator",
     icu_break_iterator_locales,
     METH_NOARGS,
    ...
    {"string_length",
     icu_string_length,
     METH_O,
     "string_length(string) -> Return the length of a string (number of unicode code points in the string). Useful on narrow python builds where len() returns "
     "an incorrect answer if the string contains surrogate pairs."},

    {"utf16_length",
     icu_utf16_length,
     METH_O,
     "utf16_length(string) -> Return the length of a string (number of UTF-16 code points in the string). Useful on wide python builds where len() returns an "
     "incorrect answer if the string contains surrogate pairs."},

    {"word_prefix_find",
     (PyCFunction)(void (*)(void))icu_word_prefix_find,
     METH_FASTCALL,
     "word_prefix_find(collator, break_iterator, string, prefix) -> Return the codepoint offset of the first word in string that starts with prefix according "
     "to collator, or -1 if none."},
...
exec_module(PyObject *mod) {
    ...
    if (PyType_Ready(&icu_CollatorType) < 0) return -1;
    if (PyType_Ready(&icu_BreakIteratorType) < 0) return -1;
    if (PyType_Ready(&icu_BreakIteratorIterType) < 0) return -1;
    if (PyType_Ready(&icu_TransliteratorType) < 0) return -1;

    Py_INCREF(&icu_CollatorType);
    Py_INCREF(&icu_BreakIteratorType);
    Py_INCREF(&icu_TransliteratorType);
...
PyInit_icu(void) { return PyModuleDef_Init(&module_def); }
