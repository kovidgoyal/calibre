...
def export(destdir, library_paths=None, dbmap=None, progress1=None, progress2=None, abort=None):
    ...
    dbmap = dbmap or {}
    dbmap = {os.path.normcase(os.path.abspath(k)):v for k, v in six.iteritems(dbmap)}
    exporter = Exporter(destdir)
    exporter.metadata['libraries'] = libraries = {}
    total = len(library_paths) + 1
    for i, (lpath, count) in enumerate(six.iteritems(library_paths)):
...
class Importer(object):
    ...
    def __init__(self, path_to_export_dir):
        ...
        self.part_map = {num:path for num, (path, is_last) in six.iteritems(part_map)}
...
def import_data(importer, library_path_map, config_location=None, progress1=None, progress2=None, abort=None):
    ...
    for i, (library_key, dest) in enumerate(six.iteritems(library_path_map)):
        if abort is not None and abort.is_set():
            return
        if progress1 is not None:
...
def run_exporter(export_dir=None, args=None):
    if args:
        ...
        all_libraries = {os.path.normcase(os.path.abspath(path)):lus for path, lus in six.iteritems(all_known_libraries())}
        if 'all' in args[1:]:
    ...
    if os.listdir(export_dir):
        raise SystemExit('%s is not empty' % export_dir)
    library_paths = {}
    for lpath, lus in six.iteritems(all_known_libraries()):
