...
DML_ERRORS = {
    ...
    'DLL_USAGE': (
        0x4004,
        'An application initialized as APPCLASS_MONITOR has attempted to perform a DDE transaction,'
    ...
    'INVALIDPARAMETER': (
        ...
        ' The application used a data handle initialized with a different item name handle than was required'
        ' by the transaction.  The application used a data handle that was initialized with a different'
        ' clipboard data format than was required by the transaction.  The application used a client-side conversation'
        ' handle with a server-side function or vice versa.  The application used a freed data handle or string handle.'
    ...
    'REENTRANCY': (
        ...
        ' another synchronous transaction, or the DdeEnableCallback function was called from within a DDEML callback function.',
    ),  # noqa: E501
    'SERVER_DIED': (
        0x400E,
    ...
    'UNFOUND_QUEUE_ID': (
        ...
        ' Once the application has returned from an XTYP_XACT_COMPLETE callback, the transaction identifier for that callback function is no longer valid.',
    ),  # noqa: E501
}
...
Initialize = cwrap(
    'DdeInitializeW',
    UINT,
    a('instance_p', LPDWORD),
    a('callback', DDECALLBACK),
    a('command', DWORD),
    a('reserved', DWORD, 0),
    errcheck=init_errcheck,
)
...
ClientTransaction = cwrap(
    'DdeClientTransaction',
    HDDEDATA,
    a('data', LPBYTE),
    a('size', DWORD),
    a('conversation', HCONV),
    a('item', HSZ),
    a('fmt', UINT, 0),
    a('type', UINT, XTYP_EXECUTE),
    a('timeout', DWORD, 5000),
    a('result', LPDWORD, LPDWORD()),
    errcheck=no_errcheck,
)
