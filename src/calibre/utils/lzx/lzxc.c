...
static int
cmp_leaves(const void *in_a, const void *in_b) {
    ...
    if (a->freq == b->freq) return a->sym - b->sym;
        ...
        do {
            ...
            do {
                ...
                if (f1 && f2) {
                    ...
                    if (f1->pathlength > f2->pathlength) next_inode->pathlength = f1->pathlength + 1;
                    else next_inode->pathlength = f2->pathlength + 1;
	...
	pathlength--;
      ...
      cur_code++;
        ...
        for (i = 0; i < nleaves; i++) {
            ...
            fprintf(stderr, "%3d: %3d %3d %-16.16s '%c'\n", i, leaves[i].freq, leaves[i].pathlength, code, leaves[i].sym);
        }
#endif
    } else if (nleaves == 1) {
...
lzx_get_chars(lz_info *lzi, int n, unsigned char *buf) {
    ...
    if (n > lzud->left_in_frame) n = lzud->left_in_frame;
    if (n > lzud->left_in_block) n = lzud->left_in_block;
#endif
    ...
    if (lzud->left_in_frame < 0) lzud->left_in_frame += LZX_FRAME_SIZE;
#endif
...
#ifdef NONSLIDE
static int
find_match_at(lz_info *lzi, int loc, int match_len, int *match_locp) {
...
#endif
static void
check_entropy(lzxc_data *lzud, int main_index) {
    ...
    if (((n & 0xFFF) == 0) && (lzud->left_in_block >= 0x1000)) {
        ...
        cur_ratio =
            (n * rn_ln2 * (n_ln_n + lzud->main_entropy) + 24 + 3 * 80 + NUM_CHARS + (lzud->main_tree_size - NUM_CHARS) * 3 + NUM_SECONDARY_LENGTHS) / (double)n;
#ifdef DEBUG_ENTROPY
        fprintf(stderr, "n = %d\n", n);
...
lzx_output_match(lz_info *lzi, int match_pos, int match_len) {
...
#ifdef DEBUG_LZ
    {
        ...
        for (i = 0; i < match_len; i++) {
            ...
            if (pos > lzi->slide_buf_size) pos -= lzi->slide_buf_size;
...
testforr:
    if (match_pos == -lzud->R0) {
        ...
        if (!btdt) {
            btdt = 1;
            if (find_match_at(lzi, lzud->R0, match_len, &match_pos) == 0) goto testforr;
            if (find_match_at(lzi, lzud->R1, match_len, &match_pos) == 0) goto testforr;
            if (find_match_at(lzi, lzud->R2, match_len, &match_pos) == 0) goto testforr;
        }

        formatted_offset = -match_pos + 2;

        if ((match_len < 3) || ((formatted_offset >= 64) && (match_len < 4)) || ((formatted_offset >= 2048) && (match_len < 5)) ||
...
#endif
                if (formatted_offset > position_base[mid]) /* too low */
                    left = mid + 1;
                else /* too high */ right = mid;
#if 0
    ...
    fprintf(
        stderr,
        "(%08x, %d, %d, %d, %d, %d)\n",
        lzud->lzi->cur_loc - lzud->lzi->chars_in_match,
        match_pos,
        match_len,
        formatted_offset,
        position_slot,
    ...
    *lzud->block_codesp++ = 0x80000000 |             /* bit 31 in intelligent bit ordering */
    ...
    if (extra_bits[position_slot] >= 3) { lzud->aligned_freq_table[position_footer & 7]++; }
#ifndef OLDFRAMING
    lzud->left_in_block -= match_len;
#endif
    if (lzud->subdivide) check_entropy(lzud, len_pos_header + NUM_CHARS);
...
lzx_output_literal(lz_info *lzi, unsigned char ch) {
    ...
    *lzud->block_codesp++ = ch;
    ...
    if (lzud->subdivide) check_entropy(lzud, ch);
}

static void
lzx_write_bits(lzxc_data *lzxd, int nbits, uint32_t bits) {
    int cur_bits;
    int shift_bits;
    int rshift_bits;
    uint16_t mask_bits;
...
static void
lzx_align_output(lzxc_data *lzxd) {
    if (lzxd->bits_in_buf) { lzx_write_bits(lzxd, 16 - lzxd->bits_in_buf, 0); }
    if (lzxd->mark_frame) lzxd->mark_frame(lzxd->mark_frame_arg, lzxd->len_uncompressed_input, lzxd->len_compressed_output);
}
...
lzx_write_compressed_literals(lzxc_data *lzxd, int block_type) {
    ...
    while (cursor < endp) {
        ...
        if (frame_count > LZX_FRAME_SIZE) {
            fprintf(
                stderr,
                "uncomp_len = %x, frame_count = %x, block_code = %08x, match_len_m2 = %d",
                lzxd->len_uncompressed_input,
                frame_count,
                block_code,
                match_len_m2);
        }
...
static int
lzx_write_compressed_tree(struct lzxc_data *lzxd, struct huff_entry *tree, uint8_t *prevlengths, int treesize) {
    unsigned char *codes;
    unsigned char *runs;
    ...
    for (i = 1; i <= treesize; i++) {
        if ((i == treesize) || (tree[i].codelength != last_len)) {
            ...
            if (i != treesize) last_len = tree[i].codelength;
            cur_run = 0;
        }
        cur_run++;
    }
    ...
    *runp++ = 255;
    ...
    for (i = 0; i < LZX_PRETREE_SIZE; i++) { lzx_write_bits(lzxd, 4, pretree[i].codelength); }
    codep = codes;
    runp = runs;
    cur_run = 0;
    while (codep < codee) {
...
int
lzxc_compress_block(lzxc_data *lzxd, int block_size, int subdivide) {
    int i;
    ...
    do {
        ...
        if (lzxd->left_in_frame == 0) lzxd->left_in_frame = LZX_FRAME_SIZE;

        if ((lzxd->subdivide < 0) || !lzxd->left_in_block || (!lz_left_to_process(lzxd->lzi) && lzxd->at_eof(lzxd->in_arg))) {
           ...
           an exact multiple of frame size */
            ...
            if (comp_bits_ovh < uncomp_bits) block_type = LZX_ALIGNED_OFFSET_BLOCK;
            else block_type = LZX_VERBATIM_BLOCK;
            ...
            if (block_type == LZX_ALIGNED_OFFSET_BLOCK) {
                for (i = 0; i < LZX_ALIGNED_SIZE; i++) { lzx_write_bits(lzxd, 3, lzxd->aligned_tree[i].codelength); }
            }
            /* end extra bits */
            ...
            lzx_write_compressed_tree(lzxd, lzxd->main_tree + NUM_CHARS, lzxd->prev_main_treelengths + NUM_CHARS, lzxd->main_tree_size - NUM_CHARS);
            ...
            for (i = 0; i < lzxd->main_tree_size; i++) { lzxd->prev_main_treelengths[i] = lzxd->main_tree[i].codelength; }
            for (i = 0; i < NUM_SECONDARY_LENGTHS; i++) { lzxd->prev_length_treelengths[i] = lzxd->length_tree[i].codelength; }
            lzxd->main_entropy = 0.0;
            lzxd->last_ratio = 9999999.0;
            lzxd->block_codesp = lzxd->block_codes;
...
lzxc_init(
    ...
    if ((wsize_code < 15) || (wsize_code > 21)) { return -1; }
    lzx_init_static();
    ...
    *lzxdp = lzxd = malloc(sizeof(*lzxd));
    ...
    lz_init(lzxd->lzi, wsize, wsize - 3, MAX_MATCH, MIN_MATCH, LZX_FRAME_SIZE, lzx_get_chars, lzx_output_match, lzx_output_literal, lzxd);
...
int
lzxc_finish(struct lzxc_data *lzxd, struct lzxc_results *lzxr) {
    ...
    if (lzxd->block_codes) { free(lzxd->block_codes); }
    free(lzxd);
    return 0;
}
