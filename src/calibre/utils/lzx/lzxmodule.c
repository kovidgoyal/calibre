...
static char lzx_doc[] = "Provide basic LZX compression and decompression using the code from\n"
...
glue_alloc(struct mspack_system *this, size_t bytes) {
    void *p = NULL;
    p = (void *)malloc(bytes);
    if (p == NULL) { return (void *)PyErr_NoMemory(); }
    return p;
}
...
glue_write(struct mspack_file *file, void *buffer, int bytes) {
    ...
    remaining = mem->total_bytes - mem->current_bytes;
    if (bytes > remaining) {
        PyErr_SetString(LZXError, "MSPACK_WRITE tried to write beyond end of buffer");
        bytes = remaining;
    }
...
decompress(PyObject *self, PyObject *args) {
    ...
    if (!PyArg_ParseTuple(args, "y#I", &inbuf, &inlen, &outlen)) { return NULL; }
    ...
    lzx_stream =
        lzxd_init(&lzxglue_system, (struct mspack_file *)&source, (struct mspack_file *)&dest, LZXwindow, 0x7fff /* Never reset, I do it */, 4096, outlen);
    err = -1;
...
PyInit_lzx(void) { return PyModuleDef_Init(&module_def); }
