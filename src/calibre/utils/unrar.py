...
def extract_member(path_or_stream, match=re.compile(r'\.(jpg|jpeg|gif|png)\s*$', re.I), name=None):
...
def extract_first_alphabetically(stream):
    ...
    names_ = sorted(
        (x for x in names(stream) if os.path.splitext(x)[1][1:].lower() in {'png', 'jpg', 'jpeg', 'gif', 'webp'}),
        key=sort_key,
    )
        ...
        for name, data in tdata.items():
            if name not in '1 2 symlink'.split():
                ...
                if d is None or d[1] != data:
                    raise ValueError(f'Failed to extract {name} {d!r} != {data!r}')
