...
class ZipFile:
    ...
    def _calculate_file_offsets(self):
        ...
        for zip_info in self.filelist:
            ...
            # lengths
            file_offset = zip_info.header_offset + 30 + fheader[_FH_FILENAME_LENGTH] + fheader[_FH_EXTRA_FIELD_LENGTH]
            ...
            if fname != zip_info.orig_filename:
                raise RuntimeError(f'File name in directory "{zip_info.orig_filename}" and header "{fname}" differ.')
    ...
    def printdir(self):
        ...
        for zinfo in self.filelist:
            date = (
                f'{zinfo.date_time[0]}-{zinfo.date_time[1]:02}-{zinfo.date_time[2]:02} {zinfo.date_time[3]:02}:{zinfo.date_time[4]:02}:{zinfo.date_time[5]:02}'
            )
    ...
    def open(self, name, mode='r', pwd=None):
        ...
        if fname != zinfo.orig_filename:
            print(f'WARNING: Header ({fname!r}) and directory ({zinfo.orig_filename!r}) filenames do not match inside ZipFile')
    ...
    def writestr(self, zinfo_or_arcname, byts, permissions=0o600, compression=ZIP_DEFLATED, raw_bytes=False):
...
def safe_replace(zipstream_or_path, name, datastream, extra_replacements=None, add_missing=False):
    ...
    if isinstance(zipstream_or_path, str):
        ...
        with (
            ZipFile(zipstream_or_path, 'r') as z,
            tempfile.NamedTemporaryFile(prefix=prefix, dir=parent_dir, delete=False) as temp,
        ):
...
def find_tests():
    ...
    class TestZipFileSafeReplace(unittest.TestCase):
        ...
        def test_safe_replace_path_extra_replacements(self):
            ...
            safe_replace(zpath, 'a.txt', io.BytesIO(b'A'), extra_replacements={'b.txt': io.BytesIO(b'B')})
        ...
        def test_safe_replace_path_preserves_file_attributes(self):
            ...
            self.assertEqual(new_stat.st_uid, original_stat.st_uid, 'File owner (uid) not preserved after safe_replace with path')
            self.assertEqual(new_stat.st_gid, original_stat.st_gid, 'File group (gid) not preserved after safe_replace with path')
        ...
        def test_safe_replace_stream_extra_replacements(self):
            ...
            buf.seek(0)
            safe_replace(buf, 'a.txt', io.BytesIO(b'A'), extra_replacements={'b.txt': io.BytesIO(b'B')})
