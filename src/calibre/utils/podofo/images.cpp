...
class Image {
  ...
  public:
    ...
    Image(Image &&other) noexcept
        : buf(std::move(other.buf)), width(other.width), height(other.height), ref(other.ref), smask(other.smask), content_hash(other.content_hash),
          overall_hash(other.overall_hash) {
        other.buf = charbuff();
    ...
    bool
    operator==(const Image &other) const noexcept {
    ...
    std::string
    ToString() const {
        return "Image(ref=" + ref.ToString() + ", width="s + std::to_string(width) + ", height="s + std::to_string(height) + ", smask="s + smask.ToString() +
               ", digest=" + std::to_string(content_hash) + ")";
    }
};

struct ImageHasher {
    std::size_t
    operator()(const Image &k) const {
        return k.hash();
    }
};
