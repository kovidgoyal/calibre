...
class WinToastHandler : public WinToastLib::IWinToastHandler {
  public:
    ...
    // Public interfaces
    void
    toastActivated() const override {}
    void
    toastActivated(int actionIndex) const override {
        wchar_t buf[250];
        swprintf_s(buf, L"Button clicked: %d", actionIndex);
    }
    void
    toastDismissed(WinToastDismissalReason state) const override {}
    void
...
initialize_toast(PyObject *self, PyObject *args) {
    ...
    if (!WinToast::instance()->initialize(&error)) {
        return set_error(
            "Failed to initialize WinToast with error: %s using appname: %S and app model id: %S", error, PyTuple_GET_ITEM(args, 0), PyTuple_GET_ITEM(args, 1));
    }
    Py_RETURN_NONE;
}
...
notify(PyObject *self, PyObject *args) {
    ...
    if (id == -1 || error != WinToast::WinToastError::NoError) {
        return set_error(
            "Failed to show notification with error: %s using title: %S and message: %S", error, PyTuple_GET_ITEM(args, 0), PyTuple_GET_ITEM(args, 1));
    }
