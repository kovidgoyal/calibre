...
// GUID {{{
typedef struct {
    PyObject_HEAD GUID guid;
} PyGUID;

static PyTypeObject PyGUIDType = {PyVarObject_HEAD_INIT(NULL, 0)};
...
static PyMethodDef PyGUID_methods[] = {{NULL, NULL, 0, NULL}};
#undef M
...
Handle_close_(Handle *self) {
    if (self->handle) {
        switch (self->handle_type) {
            case NormalHandle: CloseHandle(self->handle); break;
            case ModuleHandle: FreeLibrary((HMODULE)self->handle); break;
            case IconHandle: DestroyIcon((HICON)self->handle); break;
            case BitmapHandle: DeleteObject((HBITMAP)self->handle); break;
        }
        self->handle = NULL;
    }
}
...
Handle_repr(Handle *self) {
    ...
    switch (self->handle_type) {
        case NormalHandle: name = "HANDLE"; break;
        case ModuleHandle: name = "HMODULE"; break;
        case IconHandle: name = "HICON"; break;
        case BitmapHandle: name = "HBITMAP"; break;
    }
...
static PyTypeObject HandleType = {PyVarObject_HEAD_INIT(NULL, 0)};
...
Handle_new(PyTypeObject *type, PyObject *args, PyObject *kwds) {
    ...
    switch (htype) {
        ...
        case IconHandle:
        case ModuleHandle:
        case BitmapHandle: break;
        default: PyErr_Format(PyExc_TypeError, "unknown handle type: %d", type); return NULL;
    }
...
#define M(name, args) {#name, (PyCFunction)Handle_##name, args, ""}
static PyMethodDef Handle_methods[] = {M(close, METH_NOARGS), M(detach, METH_NOARGS), {NULL, NULL, 0, NULL}};
#undef M
// }}}
...
class DeleteFileProgressSink : public IFileOperationProgressSink { // {{{
  ...
  private:
    ...
    HRESULT STDMETHODCALLTYPE
    FinishOperations(HRESULT) {
        return S_OK;
    }
    HRESULT STDMETHODCALLTYPE
    PreRenameItem(DWORD, IShellItem *, LPCWSTR) {
        return S_OK;
    }
    HRESULT STDMETHODCALLTYPE
    ...
    HRESULT STDMETHODCALLTYPE
    PreMoveItem(DWORD, IShellItem *, IShellItem *, LPCWSTR) {
        return E_NOTIMPL;
    }
    HRESULT STDMETHODCALLTYPE
    ...
    HRESULT STDMETHODCALLTYPE
    PreCopyItem(DWORD, IShellItem *, IShellItem *, LPCWSTR) {
        return E_NOTIMPL;
    }
    HRESULT STDMETHODCALLTYPE
    ...
    HRESULT STDMETHODCALLTYPE
    PreDeleteItem(DWORD dwFlags, IShellItem *) {
        if (!(dwFlags & TSF_DELETE_RECYCLE_IF_POSSIBLE)) return E_ABORT;
        return S_OK;
    }
    HRESULT STDMETHODCALLTYPE
    PostDeleteItem(DWORD, IShellItem *, HRESULT, IShellItem *) {
        return S_OK;
    }
    HRESULT STDMETHODCALLTYPE
    PreNewItem(DWORD, IShellItem *, LPCWSTR) {
        return E_NOTIMPL;
    }
    HRESULT STDMETHODCALLTYPE
    PostNewItem(DWORD, IShellItem *, LPCWSTR, LPCWSTR, DWORD, HRESULT, IShellItem *) {
        return E_NOTIMPL;
    }
    HRESULT STDMETHODCALLTYPE
    UpdateProgress(UINT, UINT) {
        return S_OK;
    }
    HRESULT STDMETHODCALLTYPE
    ResetTimer(void) {
...
winutil_get_disk_free_space(PyObject *self, PyObject *args) {
    ...
    BOOL ok;
    Py_BEGIN_ALLOW_THREADS ok = GetDiskFreeSpaceEx(path.ptr(), &bytes_available_to_caller, &total_bytes, &total_free_bytes);
    Py_END_ALLOW_THREADS if (!ok) return PyErr_SetExcFromWindowsErrWithFilenameObject(PyExc_OSError, 0, PyTuple_GET_ITEM(args, 0));
...
winutil_get_processes_using_files(PyObject *self, PyObject *args) {
    ...
    if (dwError != ERROR_SUCCESS) {
        PyErr_SetFromWindowsErr(dwError);
        return NULL;
    }
    ...
    for (UINT i = 0; i < nProcInfo; i++) {
        ...
        if (process) {
            ...
            if (GetProcessTimes(process.ptr(), &ftCreate, &ftExit, &ftKernel, &ftUser) && CompareFileTime(&rgpi[i].Process.ProcessStartTime, &ftCreate) == 0) {
                DWORD cch = process_path.size();
                if (QueryFullProcessImageNameW(process.ptr(), 0, process_path.data(), &cch) && cch < process_path.size()) {
                    pyobject_raii pp(Py_BuildValue(
                        "{su su# si}", "app_name", rgpi[i].strAppName, "path", process_path.data(), (Py_ssize_t)cch, "app_type", (int)rgpi[i].ApplicationType));
                    if (!pp) return NULL;
...
winutil_create_file(PyObject *self, PyObject *args) {
    ...
    HANDLE h = CreateFileW(path.ptr(), desired_access, share_mode, NULL, creation_disposition, flags_and_attributes, NULL);
...
supports_hardlinks(PyObject *self, PyObject *args) {
    ...
    if (!GetVolumeInformationW(path.ptr(), NULL, 0, NULL, &max_component_length, &flags, NULL, 0))
...
filesystem_type_name(PyObject *self, PyObject *args) {
    ...
    DWORD max_component_length, flags;
    wchar_t fsname[128];
    if (!GetVolumeInformationW(path.ptr(), NULL, 0, NULL, &max_component_length, &flags, fsname, sizeof(fsname) / sizeof(fsname[0])))
        return PyErr_SetExcFromWindowsErrWithFilenameObject(PyExc_OSError, 0, PyTuple_GET_ITEM(args, 0));
...
winutil_create_hard_link(PyObject *self, PyObject *args) {
    ...
    BOOL ok;
    Py_BEGIN_ALLOW_THREADS ok = CreateHardLinkW(path.ptr(), existing_path.ptr(), NULL);
    Py_END_ALLOW_THREADS if (!ok) return PyErr_SetExcFromWindowsErrWithFilenameObjects(PyExc_OSError, 0, PyTuple_GET_ITEM(args, 0), PyTuple_GET_ITEM(args, 1));
    Py_RETURN_NONE;
}
...
create_named_pipe(PyObject *self, PyObject *args) {
    ...
    if (!PyArg_ParseTuple(
            args, "O&kkkkkk", py_to_wchar_no_none, &name, &open_mode, &pipe_mode, &max_instances, &out_buffer_size, &in_buffer_size, &default_time_out))
        return NULL;
...
wait_for_multiple_objects(PyObject *self, PyObject *args) {
    ...
    for (Py_ssize_t i = 0; i < PySequence_Fast_GET_SIZE(ph.ptr()); i++) {
        ...
        if (Py_TYPE(obj) != &HandleType) {
            PyErr_SetString(PyExc_TypeError, "handles must contain only Handle objects");
            return NULL;
        }
...
get_long_path_name(PyObject *self, PyObject *args) {
    ...
    if (!buf) return PyErr_NoMemory();
    DWORD needed_size;
    Py_BEGIN_ALLOW_THREADS needed_size = GetLongPathNameW(path.ptr(), buf.ptr(), current_size);
    Py_END_ALLOW_THREADS if (needed_size >= current_size - 32) {
...
write_file(PyObject *self, PyObject *args) {
    ...
    DWORD written = 0;
    BOOL ok;
    Py_BEGIN_ALLOW_THREADS ok = WriteFile(handle, data + offset, (DWORD)(size - offset), &written, NULL);
    Py_END_ALLOW_THREADS if (!ok) return set_error_from_handle(args);
...
wait_named_pipe(PyObject *self, PyObject *args) {
    ...
    BOOL ok;
    Py_BEGIN_ALLOW_THREADS ok = WaitNamedPipeW(path.ptr(), timeout);
    Py_END_ALLOW_THREADS if (!ok) return PyErr_SetExcFromWindowsErrWithFilenameObject(PyExc_OSError, 0, PyTuple_GET_ITEM(args, 0));
    Py_RETURN_TRUE;
}
...
load_icons(PyObject *self, PyObject *args) {
    ...
    if (resource_id == NULL) {
        PyErr_Format(PyExc_IndexError, "no resource found with index: %d in handle: %S", index, PyTuple_GET_ITEM(args, 0));
        return NULL;
    }
    ...
    if (!size) {
        PyErr_SetString(PyExc_ValueError, "the icon group resource at the specified index is empty");
        return NULL;
    }
    HGLOBAL hglob = LoadResource(handle, res);
    if (hglob == NULL) return set_error_from_handle(args);
    ...
    if (!grp_icon_dir) {
        PyErr_SetString(PyExc_RuntimeError, "failed to lock icon group resource");
        return NULL;
    }
    PyObject *ans = PyList_New(0);
    if (!ans) return NULL;
...
get_icon_for_file(PyObject *self, PyObject *args) {
    ...
    HRESULT hr;
    Py_BEGIN_ALLOW_THREADS hr = SHCreateItemFromParsingName(path.ptr(), NULL, IID_PPV_ARGS(&pImageFactory));
    Py_END_ALLOW_THREADS if (!SUCCEEDED(hr)) { return error_from_hresult(hr, "Failed to create Shell Item from path"); }
    SIZE size = {width, height};
    HBITMAP hbmp;
    Py_BEGIN_ALLOW_THREADS hr = pImageFactory->GetImage(size, SIIGBF_BIGGERSIZEOK | SIIGBF_SCALEUP | SIIGBF_ICONONLY, &hbmp);
    pImageFactory->Release();
    Py_END_ALLOW_THREADS if (!SUCCEEDED(hr)) { return error_from_hresult(hr, "Failed to get image from shell item"); }
    BITMAP bmp;
...
static PyMethodDef winutil_methods[] = {
    ...
    {"internet_connected", winutil_internet_connected, METH_VARARGS, "internet_connected()\n\nReturn True if there is an active internet connection"},

    {"prepare_for_restart",
     winutil_prepare_for_restart,
     METH_VARARGS,
     "prepare_for_restart()\n\nRedirect output streams so that the child process does not lock the temp files"},

    {"getmaxstdio", winutil_get_max_stdio, METH_VARARGS, "getmaxstdio()\n\nThe maximum number of open file handles."},

    {"setmaxstdio", winutil_set_max_stdio, METH_VARARGS, "setmaxstdio(num)\n\nSet the maximum number of open file handles."},

    {"username", (PyCFunction)winutil_username, METH_NOARGS, "username()\n\nGet the current username as a unicode string."},

    {"temp_path", (PyCFunction)winutil_temp_path, METH_NOARGS, "temp_path()\n\nGet the current temporary dir as a unicode string."},

    {"locale_name", (PyCFunction)winutil_locale_name, METH_NOARGS, "locale_name()\n\nGet the current locale name as a unicode string."},

    {"localeconv", (PyCFunction)winutil_localeconv, METH_NOARGS, "localeconv()\n\nGet the locale conventions as unicode strings."},

    {"move_file", (PyCFunction)winutil_move_file, METH_VARARGS, "move_file()\n\nRename the specified file."},

    {"add_to_recent_docs", (PyCFunction)winutil_add_to_recent_docs, METH_VARARGS, "add_to_recent_docs()\n\nAdd a path to the recent documents list"},

    {"file_association",
     (PyCFunction)winutil_file_association,
     METH_VARARGS,
     "file_association()\n\nGet the executable associated with the given file extension"},

    {"friendly_name", (PyCFunction)winutil_friendly_name, METH_VARARGS, "friendly_name()\n\nGet the friendly name for the specified prog_id/exe"},

    {"notify_associations_changed",
     (PyCFunction)winutil_notify_associations_changed,
     METH_VARARGS,
     "notify_associations_changed()\n\nNotify the OS that file associations have changed"},

    {"move_to_trash", (PyCFunction)winutil_move_to_trash, METH_VARARGS, "move_to_trash()\n\nMove the specified path to trash"},

    {"manage_shortcut", (PyCFunction)winutil_manage_shortcut, METH_VARARGS, "manage_shortcut()\n\nManage a shortcut"},

    {"resolve_lnk", (PyCFunction)resolve_lnk, METH_VARARGS, "resolve_lnk()\n\nGet the target of a lnk file."},

    {"get_file_id",
     (PyCFunction)winutil_get_file_id,
     METH_VARARGS,
     "get_file_id(path)\n\nGet the windows file id (volume_num, file_index_high, file_index_low)"},

    {"create_file",
     (PyCFunction)winutil_create_file,
     METH_VARARGS,
     "create_file(path, desired_access, share_mode, creation_disposition, flags_and_attributes)\n\nWrapper for CreateFile"},

    {"get_file_size", (PyCFunction)winutil_get_file_size, METH_VARARGS, "get_file_size(handle)\n\nWrapper for GetFileSizeEx"},

    {"set_file_pointer", (PyCFunction)winutil_set_file_pointer, METH_VARARGS, "set_file_pointer(handle, pos, method=FILE_BEGIN)\n\nWrapper for SetFilePointer"},

    {"set_file_handle_delete_on_close",
     (PyCFunction)winutil_set_file_handle_delete_on_close,
     METH_VARARGS,
    ...
    {"read_file", (PyCFunction)winutil_read_file, METH_VARARGS, "read_file(handle, chunk_size=16KB)\n\nWrapper for ReadFile"},

    {"get_disk_free_space", (PyCFunction)winutil_get_disk_free_space, METH_VARARGS, "get_disk_free_space(path)\n\nWrapper for GetDiskFreeSpaceEx"},

    {"delete_file", (PyCFunction)winutil_delete_file, METH_VARARGS, "delete_file(path)\n\nWrapper for DeleteFile"},

    {"create_hard_link", (PyCFunction)winutil_create_hard_link, METH_VARARGS, "create_hard_link(path, existing_path)\n\nWrapper for CreateHardLink"},

    {"nlinks", (PyCFunction)winutil_nlinks, METH_VARARGS, "nlinks(path)\n\nReturn the number of hardlinks"},

    {"set_file_attributes", (PyCFunction)winutil_set_file_attributes, METH_VARARGS, "set_file_attributes(path, attrs)\n\nWrapper for SetFileAttributes"},

    {"move_file", (PyCFunction)winutil_move_file, METH_VARARGS, "move_file(a, b, flags)\n\nWrapper for MoveFileEx"},

    {"read_directory_changes",
     (PyCFunction)winutil_read_directory_changes,
     METH_VARARGS,
     "read_directory_changes(handle, buffer, subtree, flags)\n\nWrapper for ReadDirectoryChangesW"},

    {"get_processes_using_files",
     (PyCFunction)winutil_get_processes_using_files,
     METH_VARARGS,
...
#define add_type(name, doc, obj)                                    \
    obj##Type.tp_name = "winutil." #name;                           \
    obj##Type.tp_doc = doc;                                         \
    obj##Type.tp_basicsize = sizeof(obj);                           \
    obj##Type.tp_itemsize = 0;                                      \
    obj##Type.tp_flags = Py_TPFLAGS_DEFAULT;                        \
    obj##Type.tp_repr = (reprfunc)obj##_repr;                       \
    obj##Type.tp_str = (reprfunc)obj##_repr;                        \
    obj##Type.tp_new = obj##_new;                                   \
    obj##Type.tp_dealloc = (destructor)obj##_dealloc;               \
    obj##Type.tp_methods = obj##_methods;                           \
    if (PyType_Ready(&obj##Type) < 0) { return -1; }                \
    Py_INCREF(&obj##Type);                                          \
    if (PyModule_AddObject(m, #name, (PyObject *)&obj##Type) < 0) { \
        Py_DECREF(&obj##Type);                                      \
        return -1;                                                  \
    }
...
#define A(name)                                                \
    {                                                          \
        PyObject *g = create_guid(FOLDERID_##name);            \
        if (!g) { return -1; }                                 \
        if (PyModule_AddObject(m, "FOLDERID_" #name, g) < 0) { \
            Py_DECREF(g);                                      \
            return -1;                                         \
        }                                                      \
    }
...
#define A(name) \
    if (PyModule_AddIntConstant(m, #name, name) != 0) { return -1; }
    ...
    A(PIPE_ACCESS_OUTBOUND);
    A(PIPE_ACCESS_DUPLEX);
