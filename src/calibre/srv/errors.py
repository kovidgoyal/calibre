...
class HTTPRedirect(HTTPSimpleResponse):
    ...
    def __init__(self, location, http_code=six.moves.http_client.MOVED_PERMANENTLY, http_message='', close_connection=False):
...
class HTTPNotFound(HTTPSimpleResponse):
    ...
    def __init__(self, http_message='', close_connection=False):
        HTTPSimpleResponse.__init__(self, six.moves.http_client.NOT_FOUND, http_message, close_connection)


class HTTPAuthRequired(HTTPSimpleResponse):

    def __init__(self, payload, log=None):
        HTTPSimpleResponse.__init__(self, six.moves.http_client.UNAUTHORIZED, authenticate=payload, log=log)


class HTTPBadRequest(HTTPSimpleResponse):

    def __init__(self, message, close_connection=False):
        HTTPSimpleResponse.__init__(self, six.moves.http_client.BAD_REQUEST, message, close_connection)


class HTTPForbidden(HTTPSimpleResponse):

    def __init__(self, http_message='', close_connection=True, log=None):
        HTTPSimpleResponse.__init__(self, six.moves.http_client.FORBIDDEN, http_message, close_connection, log=log)


class HTTPInternalServerError(HTTPSimpleResponse):

    def __init__(self, http_message='', close_connection=True, log=None):
        HTTPSimpleResponse.__init__(self, six.moves.http_client.INTERNAL_SERVER_ERROR, http_message, close_connection, log=log)
