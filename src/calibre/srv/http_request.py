...
def parse_uri(uri, parse_query=True):
    ...
    if path is None:
        raise HTTPSimpleResponse(six.moves.http_client.BAD_REQUEST, "No path component")
    if b'#' in path:
        raise HTTPSimpleResponse(six.moves.http_client.BAD_REQUEST, "Illegal #fragment in Request-URI.")

    if scheme:
        try:
            scheme = scheme.decode('ascii')
        except ValueError:
            raise HTTPSimpleResponse(six.moves.http_client.BAD_REQUEST, 'Un-decodeable scheme')
    ...
    if parse_query:
        try:
            ...
            raise HTTPSimpleResponse(six.moves.http_client.BAD_REQUEST, 'Unparseable query string')
    else:
        query = None
    ...
    try:
        ...
        raise HTTPSimpleResponse(six.moves.http_client.BAD_REQUEST, as_unicode(e))
...
class HTTPRequest(Connection):
    ...
    def readline(self, buf):
        ...
        if line.endswith(b'\n'):
            ...
            if not line.endswith(b'\r\n'):
                self.simple_response(six.moves.http_client.BAD_REQUEST, 'HTTP requires CRLF line terminators')
                return
            return line
        if not line:
    ...
    def connection_ready(self):
        ...
        self.header_line_too_long_error_code = six.moves.http_client.REQUEST_URI_TOO_LONG
    ...
    def parse_request_line(self, buf, event, first=False):  # {{{
        ...
        if line == b'\r\n':
            ...
            return self.simple_response(six.moves.http_client.BAD_REQUEST, 'Multiple leading empty lines not allowed')
        ...
        try:
            ...
            return self.simple_response(six.moves.http_client.BAD_REQUEST, "Malformed Request-Line")

        if self.method not in HTTP_METHODS:
            return self.simple_response(six.moves.http_client.BAD_REQUEST, "Unknown HTTP method")

        try:
            self.request_protocol = protocol_map[rp]
        except KeyError:
            return self.simple_response(six.moves.http_client.HTTP_VERSION_NOT_SUPPORTED)
        ...
        self.header_line_too_long_error_code = six.moves.http_client.REQUEST_ENTITY_TOO_LARGE
    ...
    def parse_header_line(self, parser, buf, event):
        ...
        try:
            ...
            self.simple_response(six.moves.http_client.BAD_REQUEST, 'Failed to parse header line')
            return
        if parser.finished:
    ...
    def finalize_headers(self, inheaders):
        ...
        if request_content_length > self.max_request_body_size:
            return self.simple_response(six.moves.http_client.REQUEST_ENTITY_TOO_LARGE,
        ...
        if te:
            for enc in te:
                if enc == "chunked":
                    ...
                    return self.simple_response(six.moves.http_client.NOT_IMPLEMENTED, "Unknown transfer encoding: %r" % enc)
    ...
    def read_chunk_length(self, inheaders, line_buf, buf, bytes_read, event):
        ...
        try:
            ...
            return self.simple_response(six.moves.http_client.BAD_REQUEST, '%s is not a valid chunk size' % reprlib.repr(line.strip()))
    ...
    def read_chunk_separator(self, inheaders, line_buf, buf, bytes_read, event, last=False):
        ...
        if line is None:
            return
        if line != b'\r\n':
            return self.simple_response(six.moves.http_client.BAD_REQUEST, 'Chunk does not have trailing CRLF')
        bytes_read[0] += len(line)
    ...
    def handle_timeout(self):
        if self.response_started:
            return False
        self.simple_response(six.moves.http_client.REQUEST_TIMEOUT)
        return True
