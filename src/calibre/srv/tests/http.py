...
import six.moves.http_client, hashlib, zlib, string, time, os
...
class TestHTTP(BaseTest):
    ...
    def test_http_basic(self):  # {{{
        ...
        def raw_send(conn, raw):
            conn.send(raw)
            conn._HTTPConnection__state = six.moves.http_client._CS_REQ_SENT
        ...
        with TestServer(handler, timeout=base_timeout, max_header_line_size=100./1024, max_request_body_size=100./(1024*1024)) as server:
            ...
            self.ae(r.status, six.moves.http_client.BAD_REQUEST)
            self.ae(r.read(), b'HTTP requires CRLF line terminators')

            r = raw_send(conn, b'\r\nGET /index.html HTTP/1.1\r\n\r\n')
            self.ae(r.status, six.moves.http_client.NOT_FOUND), self.ae(r.read(), b'Requested resource not found')
            ...
            self.ae(r.status, six.moves.http_client.BAD_REQUEST), self.ae(r.read(), b'Unknown HTTP method')
            ...
            r = conn.getresponse()
            self.ae(r.status, six.moves.http_client.INTERNAL_SERVER_ERROR)
            ...
            conn.request('GET', '/')
            r = conn.getresponse()
            self.ae(r.status, six.moves.http_client.MOVED_PERMANENTLY)
            ...
            conn.request('GET', '/test', ('a' * 200))
            r = conn.getresponse()
            self.ae(r.status, six.moves.http_client.REQUEST_ENTITY_TOO_LARGE)
            ...
            conn.send(b'3\r\nbody\r\n0\r\n\r\n')
            r = conn.getresponse()
            self.ae(r.status, six.moves.http_client.BAD_REQUEST), self.ae(r.read(), b'Chunk does not have trailing CRLF')

            conn = server.connect(timeout=base_timeout * 5)
            conn.request('POST', '/test', headers={'Transfer-Encoding': 'chunked'})
            conn.send(b'30\r\nbody\r\n0\r\n\r\n')
            r = conn.getresponse()
            self.ae(r.status, six.moves.http_client.REQUEST_TIMEOUT)
    ...
    def test_http_response(self):  # {{{
        ...
        with NamedTemporaryFile(suffix='test.epub') as f, open(P('localization/locales.zip'), 'rb') as lf, \
            ...
            r = conn.getresponse()
            self.ae(r.status, six.moves.http_client.OK), self.ae(r.read(), b'an_etagged_path')
            ...
            self.ae(r.status, six.moves.http_client.OK), self.ae(zlib.decompress(r.read(), 16+zlib.MAX_WBITS), raw)
            ...
            conn.request('GET', '/an_etagged_path')
            r = conn.getresponse()
            self.ae(r.status, six.moves.http_client.OK), self.ae(r.read(), b'data')
            etag = r.getheader('ETag')
            self.ae(etag, b'"xxx"')
            ...
            for use_sendfile in (True, False):
                ...
                self.ae(r.status, six.moves.http_client.OK), self.ae(r.read(), fdata)

                conn.request('GET', '/test', headers={'Range':'bytes=2-25'})
                r = conn.getresponse()
                self.ae(r.status, six.moves.http_client.PARTIAL_CONTENT)
                self.ae(type('')(r.getheader('Accept-Ranges')), 'bytes')
                ...
                r = conn.getresponse()
                self.ae(r.status, six.moves.http_client.REQUESTED_RANGE_NOT_SATISFIABLE)
                self.ae(type('')(r.getheader('Content-Range')), 'bytes */%d' % len(fdata))

                conn.request('GET', '/test', headers={'Range':'bytes=25-50', 'If-Range':etag})
                r = conn.getresponse()
                self.ae(r.status, six.moves.http_client.PARTIAL_CONTENT), self.ae(r.read(), fdata[25:51])
                self.ae(int(r.getheader('Content-Length')), 26)

                conn.request('GET', '/test', headers={'Range':'bytes=0-1000000'})
                r = conn.getresponse()
                self.ae(r.status, six.moves.http_client.PARTIAL_CONTENT), self.ae(r.read(), fdata)
