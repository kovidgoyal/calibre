...
static void
initialize_tables(State *state) {
    int32_t i, j;
    for (i = 0; i < 256; i++) {
        state->p[i] = default_ztable[i].p;
        state->m[i] = default_ztable[i].m;
        state->up[i] = default_ztable[i].up;
        state->dn[i] = default_ztable[i].dn;
        j = i;
        while (j & 0x80) {
            state->ffzt[i] += 1;
            j <<= 1;
...
decode_sub_simple(State *state, int32_t mps, uint32_t z) {
    int32_t shift = 0;
...
static inline int32_t
zpcodec_decoder(State *state) {
    return decode_sub_simple(state, 0, 0x8000 + (state->a >> 1));
}

static inline int32_t
decode_raw(State *state, int32_t bits) {
...
static inline int32_t
decode_sub(State *state, uint8_t *ctx, int32_t index, uint32_t z) {
    /* Save bit */
    ...
    if (z > state->code) {
        ...
        /* MPS adaptation */
        if (state->a >= state->m[ctx[index]]) ctx[index] = state->up[ctx[index]];
        /* MPS renormalization */
        state->scount -= 1;
        state->a = (uint16_t)(z << 1);
        state->code = (uint16_t)(state->code << 1) | ((state->buffer >> state->scount) & 1);
...
static inline int32_t
zpcodec_decode(State *state, uint8_t *ctx, int32_t index) {
...
static inline int32_t
decode_binary(State *state, uint8_t *ctx, int32_t index, int bits) {
...
static bool
decode(State *state, uint8_t *ctx) {
    uint8_t mtf[256] = {// {{{
    ...
    rotate:
        ...
        for (k = mtfno; k >= FREQMAX; k--) mtf[k] = mtf[k - 1];
    ...
    if (markerpos < 1 || (uint32_t)markerpos >= state->xsize) {
        CORRUPT;
        goto end;
    }
    // Allocate pointers
...
bzz_decompress(PyObject *self, PyObject *args) {
    ...
    if (!PyArg_ParseTuple(args, "y#", &(state.raw), &input_len)) return NULL;
