...
class SecondaryIndexHeader:  # {{{
    def __init__(self, record):
        ...
        if self.index_encoding == 'unknown':
            raise ValueError(f'Unknown index encoding: {self.index_encoding_num}')
    ...
    def __str__(self):
        ...
        def u(w):
            a('Unknown: {!r} ({} bytes) (All zeros: {!r})'.format(w, len(w), not bool(w.replace(b'\0', b''))))
...
class Tag:  # {{{
    ...
    TAG_MAP = {
        ...
        11: ('secondary', '[unknown, unknown, tag type from TAGX in primary index header]'),
        21: ('parent_index', 'Parent'),
        22: ('first_child_index', 'First child'),
        23: ('last_child_index', 'Last child'),
        69: (
            'image_index',
            'Offset from first image record to the image record associated with this entry (masthead for periodical or thumbnail for article entry).',
        ),
...
class FontRecord:  # {{{
    def __init__(self, idx, record):
        ...
        if self.font['err']:
            raise ValueError('Failed to read font record: {} Headers: {}'.format(self.font['err'], self.font['headers']))
...
class TBSIndexing:  # {{{
    def __init__(self, text_records, indices, doc_type):
        ...
        for r in text_records:
            ...
            self.record_indices[r] = {'starts': starts, 'ends': ends, 'complete': complete, 'geom': (start, end)}
    ...
    def interpret_periodical(self, tbs_type, byts, record_offset):
        ...
        def read_section_transitions(byts, psi=None):  # {{{
            ...
            while byts:
                ...
                if extra.get(0b0010, None) is not None:
                    raise ValueError("Don't know how to interpret flag 0b0010 while reading section transitions")
                if extra.get(0b1000, None) is not None:
                    if len(extra) > 1:
                        raise ValueError(f"Don't know how to interpret flags {extra!r} while reading section transitions")
                ...
                if offset is not None:
                    if offset == 0:
                        ...
                        ans.append(
                            f'->Offset to start of next section ({psi.index + 1}) from start of record: {offset} [{offset + record_offset} absolute offset]'
                        )
            return byts
        ...
        def read_starting_section(byts):  # {{{
            ...
            if len(extra) > 1 or 0b0010 in extra or 0b1000 in extra:
                raise ValueError(f"Don't know how to interpret flags {extra!r} when reading starting section")
            si = self.get_index(si)
            ans.append(f'The section at the start of this record is: {si.index}')
            if 0b0100 in extra:
                num = extra[0b0100]
                ...
                eof = extra[0b0001]
                if eof != 0:
                    raise ValueError(f'Unknown eof value {eof} when reading starting section. All bytes: {orig!r}')
                ans.append(f'??This record has more than one article from  the section: {si.index}')
            return si, byts
...
class MOBIFile:  # {{{
    def __init__(self, mf: HeadersMOBIFile):
        ...
        self.text_records = [
            TextRecord(r, self.records[r], self.mobi_header.extra_data_flags, mf.decompress6) for r in range(1, min(len(self.records), ntr + 1))
        ]
...
def inspect_mobi(mobi_file, ddir):
    ...
    with open(os.path.join(ddir, 'pretty.html'), 'wb') as of:
        of.write(etree.tostring(root, pretty_print=True, encoding='utf-8', method='html'))
