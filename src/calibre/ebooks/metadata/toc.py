...
class TOC(list):
    ...
    def add_item(self, href, fragment, text, play_order=None, type='unknown', author=None, description=None, toc_thumbnail=None):
        ...
        def process_navpoint(np, dest):
            ...
            if nl:
                ...
                for txt in txt_path(nl):
                    text += etree.tostring(txt, method='text', encoding='unicode', with_tail=False)
    ...
    def render(self, stream, uid):
        ...
        def navpoint(parent, np):
            ...
            elem = E.navPoint(
                ...
                E.content(src=str(np.href) + (('#' + str(np.fragment)) if np.fragment else '')),
                id=item_id,
