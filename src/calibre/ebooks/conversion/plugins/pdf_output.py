...
UNITS = ('millimeter', 'centimeter', 'point', 'inch', 'pica', 'didot', 'cicero', 'devicepixel')

PAPER_SIZES = ('a0', 'a1', 'a2', 'a3', 'a4', 'a5', 'a6', 'b0', 'b1', 'b2', 'b3', 'b4', 'b5', 'b6', 'legal', 'letter')
...
class PDFOutput(OutputFormatPlugin):
    ...
    options = {
        ...
        OptionRecommendation(
            ...
            short_switch='u',
            choices=UNITS,
            help=_('The unit of measure for page sizes. Default is inch. Choices are {} Note: This does not override the unit for margins!').format(
                ', '.join(UNITS)
            ),
        ),
        OptionRecommendation(
            ...
            level=OptionRecommendation.LOW,
            choices=PAPER_SIZES,
            help=_('The size of the paper. This size will be overridden when a non default output profile is used. Default is letter. Choices are {}').format(
                ', '.join(PAPER_SIZES)
            ),
        ),
        OptionRecommendation(
        ...
        OptionRecommendation(
            ...
            recommended_value=False,
            help=_('Preserve the aspect ratio of the cover, instead of stretching it to fill the full first page of the generated PDF.'),
        ),
        OptionRecommendation(
            name='pdf_serif_family',
            recommended_value='Times',
            help=_('The font family used to render serif fonts. Will work only if the font is available system-wide.'),
        ),
        OptionRecommendation(
            name='pdf_sans_family',
            recommended_value='Helvetica',
            help=_('The font family used to render sans-serif fonts. Will work only if the font is available system-wide.'),
        ),
        OptionRecommendation(
            name='pdf_mono_family',
            recommended_value='Courier',
            help=_('The font family used to render monospace fonts. Will work only if the font is available system-wide.'),
        ),
        OptionRecommendation(
            name='pdf_standard_font',
            choices=ui_data['font_types'],
            recommended_value='serif',
            help=_('The type of font family used to render font for which no font family is specified.'),
        ),
        OptionRecommendation(name='pdf_default_font_size', recommended_value=20, help=_('The default font size (in pixels)')),
        OptionRecommendation(
            name='pdf_mono_font_size',
            recommended_value=16,
            help=_('The default font size for monospaced text (in pixels)'),
        ),
        OptionRecommendation(
            name='pdf_hyphenate',
            recommended_value=False,
        ...
        OptionRecommendation(
            ...
            recommended_value=None,
            help=_(
                'An HTML template used to generate %s on every page.'
        ...
        OptionRecommendation(name='toc_title', recommended_value=None, help=_('Title for generated table of contents.')),
        OptionRecommendation(
            name='pdf_page_margin_left',
            recommended_value=72.0,
            level=OptionRecommendation.LOW,
            help=_('The size of the left page margin, in pts. Default is 72pt. Overrides the common left page margin setting.'),
        ),
        OptionRecommendation(
            name='pdf_page_margin_top',
            recommended_value=72.0,
            level=OptionRecommendation.LOW,
            help=_('The size of the top page margin, in pts. Default is 72pt. Overrides the common top page margin setting, unless set to zero.'),
        ),
        OptionRecommendation(
            name='pdf_page_margin_right',
            recommended_value=72.0,
            level=OptionRecommendation.LOW,
            help=_('The size of the right page margin, in pts. Default is 72pt. Overrides the common right page margin setting, unless set to zero.'),
        ),
        OptionRecommendation(
            name='pdf_page_margin_bottom',
            recommended_value=72.0,
            level=OptionRecommendation.LOW,
            help=_('The size of the bottom page margin, in pts. Default is 72pt. Overrides the common bottom page margin setting, unless set to zero.'),
        ),
        OptionRecommendation(
        ...
        OptionRecommendation(
            name='pdf_no_cover',
            recommended_value=False,
            help=_('Do not insert the book cover as an image at the start of the document. If you use this option, the book cover will be discarded.'),
        ),
    }
    ...
    def convert(self, oeb_book, output, input_plugin, opts, log):
        ...
        package = etree.Element(OPF('package'), attrib={'version': '2.0', 'unique-identifier': 'dummy'}, nsmap={None: OPF2_NS})
