...
def files_data(container, *args):
    for name, path in six.iteritems(container.name_path_map):
...
def words_data(container, book_locale, *args):
    ...
    return (count, tuple(Word(i, word, locale, v) for i, ((word, locale), v) in enumerate(six.iteritems(words))))
...
def chars_data(container, book_locale, *args):
    ...
    for i, (codepoint, usage) in enumerate(six.iteritems(cc.chars)):
...
def css_data(container, book_locale, result_data, *args):
    ...
    for name, inline_sheets in six.iteritems(html_sheets):
        root = container.parsed(name)
        ...
        for cls, elem_map in six.iteritems(cmap):
            class_elements = class_map[cls][name]
            for elem, usage in six.iteritems(elem_map):
                class_elements.append(
                    ClassElement(name, elem.sourceline, elem.get('class'), tag_text(elem), tuple(usage)))

    result_data['classes'] = ans = []
    for cls, name_map in six.iteritems(class_map):
        la = tuple(ClassFileMatch(name, tuple(class_elements), numeric_sort_key(name)) for name, class_elements in six.iteritems(name_map) if class_elements)
    ...
    ans = []
    for rule, loc_map in six.iteritems(rule_map):
        la = tuple(CSSFileMatch(name, tuple(locations), numeric_sort_key(name)) for name, locations in six.iteritems(loc_map) if locations)
