...
def check_opf(container):
    ...
    errors.extend(DuplicateHref(container.opf_name, eid, locs) for eid, locs in six.iteritems(dups))
    ...
    errors.extend(DuplicateHref(container.opf_name, eid, locs, for_spine=True) for eid, locs in six.iteritems(dups))
    ...
    if spine:
        ...
        if spine:
            ...
            if ncx:
                ncx_name = ncx[0]
                rmap = {v:k for k, v in six.iteritems(container.manifest_id_map)}
