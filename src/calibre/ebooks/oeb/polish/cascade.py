...
def iterdeclaration(decl):
    for p in all_properties(decl):
        ...
        if n is None:
            ...
            for k, v in six.iteritems(n(p.name, p.propertyValue)):
...
def resolve_pseudo_declarations(decls):
    ...
    return {k:resolve_declarations(v) for k, v in six.iteritems(groups)}
...
def resolve_styles(container, name, select=None, sheet_callback=None):
    ...
    for l in (style_map, pseudo_style_map):
        for x in six.itervalues(l):
            x.sort(key=itemgetter(0), reverse=True)

    style_map = {elem:resolve_declarations(x) for elem, x in six.iteritems(style_map)}
    pseudo_style_map = {elem:resolve_pseudo_declarations(x) for elem, x in six.iteritems(pseudo_style_map)}
...
def defvals():
    ...
    if _defvals is None:
        u = type('')
        _defvals = {k:Values(Property(k, u(val)).propertyValue) for k, val in six.iteritems(DEFAULTS)}
    return _defvals
