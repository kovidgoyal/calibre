...
class KepubifyTests(BaseTest):
    def test_kepubify(self):
        def b(has_cover=True, epub_version='3'):
            ...
            for q in (
                f'{CSS_COMMENT_COOKIE}: @page',
                f'-{CSS_COMMENT_COOKIE}-widows',
                f'-{CSS_COMMENT_COOKIE}-orphans',
            ):
                self.assertIn(q, ps)
            ...
            self.assertEqual(
                tuple(expected.manifest_items_with_property('cover-image')),
                tuple(actual.manifest_items_with_property('cover-image')),
            )
...
<body><div id="book-columns"><div id="book-inner">'''
        ...
        for src, expected in {
            # basics
            '<p>one</p>  <p>\xa0</p><p>\xa0<i>a</i></p>': '<p><span class="koboSpan" id="kobo.1.1">one</span>'
            ...
            'An image</span><span class="koboSpan" id="kobo.2.1"><img src="x"/></span>'
            ...
            # comments
            '<p>A comment<!-- xx -->with tail<p>A comment<!-- xx --><i>without tail': '<p><span class="koboSpan" id="kobo.1.1">A comment</span>'
            '<!-- xx --><span class="koboSpan" id="kobo.1.2">with tail</span></p>'
