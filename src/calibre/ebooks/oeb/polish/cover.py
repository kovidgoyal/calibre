...
def find_cover_image2(container, strict=False):
    ...
    for ref_type, name in six.iteritems(guide_type_map):
...
def mark_as_cover_epub(container, name):
    mmap = {v:k for k, v in six.iteritems(container.manifest_id_map)}
    if name not in mmap:
...
def create_epub_cover(container, cover_path, existing_image, options=None):
    ...
    if existing_image:
        ...
        manifest_id = {v:k for k, v in six.iteritems(container.manifest_id_map)}[existing_image]
...
def set_epub_cover(container, cover_path, report, options=None):
    ...
    link_sub = {s:d for s, d in six.iteritems({
