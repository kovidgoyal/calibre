...
''').format(
            _('%prog [options]'),
            '%prog -e myscript.py -- --option1 --option2 file1 file2 ...',
    ...
    parser.add_option('-d', '--debug-device-driver', default=False, action='store_true', help=_('Debug device detection'))
    parser.add_option(
        '-g',
        '--gui',
        default=False,
        action='store_true',
        help=_(
            'Run the GUI with debugging enabled. Debug output is '
    ...
    parser.add_option(
        '--gui-debug',
        default=None,
        help=_('Run the GUI with a debug console, logging to the specified path. For internal use only, use the -g option to run the GUI in debug mode'),
    )
    ...
    parser.add_option(
        '--add-simple-plugin',
        default=None,
        help=_('Add a simple plugin (i.e. a plugin that consists of only a .py file), by specifying the path to the py file containing the plugin code.'),
    )
    parser.add_option(
        '-m',
        '--inspect-mobi',
    ...
    parser.add_option(
        ...
        action='store_true',
        help=_(
            'Explode the book into the specified folder.\nUsage: '
    ...
    parser.add_option(
        ...
        action='store_true',
        help=_(
            'Export all calibre data (books/settings/plugins). Normally, you will'
    ...
    parser.add_option('--import-calibre-data', default=False, action='store_true', help=_('Import previously exported calibre data'))
    parser.add_option(
        '-s',
    ...
    parser.add_option('--test-build', help=_('Test binary modules in build'), action='store_true', default=False)
    parser.add_option(
        '-r',
        '--run-plugin',
        help=_(
    ...
    parser.add_option(
        ...
        default=False,
        action='store_true',
        help=_('Convert the specified EPUB file to KEPUB without doing a full conversion. This is what the Kobo driver does when sending files to the device.'),
    )
    parser.add_option(
        '--un-kepubify',
        default=False,
...
def print_basic_debug_info(out=None):
    ...
    out(__appname__, get_version(), 'Portable' if isportable else '', 'embedded-python:', isfrozen)
    ...
    if has_external_plugins():
        ...
        names = (f'{p.name} {p.version}' for p in initialized_plugins() if getattr(p, 'installation_type', None) is not PluginInstallationType.BUILTIN)
