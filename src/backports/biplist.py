...
class PlistReader(object):
        ...
        try:
            ...
            if pow(2, self.trailer.offsetSize * 8) < self.trailer.offsetTableOffset:
                raise InvalidPlistException(
                    "Offset size insufficient to reference all objects."
                )

            if pow(2, self.trailer.objectRefSize * 8) < self.trailer.offsetCount:
                raise InvalidPlistException(
                    "Too many offsets to represent in size of object reference representation."
                )
            ...
            if self.trailer.offsetSize > 16:
                raise InvalidPlistException(
                    "Offset size is greater than maximum integer size."
                )
            ...
            if self.trailer.topLevelObjectNumber >= self.trailer.offsetCount:
                raise InvalidPlistException(
                    "Top level object number is larger than the number of objects."
                )
    ...
    def setCurrentOffsetToObjectNumber(self, objectNumber):
        ...
        if self.currentOffset in self.offsetsStack:
            raise InvalidPlistException(
                "Recursive data structure detected in object: %d" % objectNumber
            )
    ...
    def readObject(self):
        ...
        if len(tmp_byte) != 1:
            raise InvalidPlistException(
                "No object found at offset: %d" % self.currentOffset
            )
        ...
        if format == 0b0000:
            if extra == 0b0000:
                ...
                raise InvalidPlistException(
                    "Invalid object found at offset: %d" % (self.currentOffset - 1)
                )
        # int
        elif format == 0b0001:
        ...
        # dict
            ...
            raise InvalidPlistException(
                "Invalid object found: {format: %s, extra: %s}" %
    ...
    def readRefs(self, count):
        ...
        i = 0
        while i < count:
            fragment = self.readContents(
                self.trailer.objectRefSize, "Object reference"
            )
    ...
    def readArray(self, count):
        if not isinstance(count, (int, long)):
            raise InvalidPlistException(
                "Count of entries in dict isn't of integer type."
            )
        result = []
    ...
    def readDict(self, count):
        if not isinstance(count, (int, long)):
            raise InvalidPlistException(
                "Count of keys/values in dict isn't of integer type."
            )
        result = {}
    ...
    def readAsciiString(self, length):
        if not isinstance(length, (int, long)):
            raise InvalidPlistException(
                "Length of ASCII string isn't of integer type."
            )
    ...
    def readUnicode(self, length):
        if not isinstance(length, (int, long)):
            raise InvalidPlistException(
                "Length of Unicode string isn't of integer type."
            )
        ...
        # 1, 2, and 4 byte integers are unsigned
            ...
            if hasattr(int, 'from_bytes'):
                ...
                for byte in data:
                    if not isinstance(
                        byte, int
                    ):  # Python3.0-3.1.x return ints, 2.x return str
...
class StringWrapper(object):
    ...
    def __new__(cls, value):
        ...
        raise ValueError(
            'Unable to get ascii or utf_16_be encoding for %s' % repr(value)
        )
...
class PlistWriter(object):
    ...
    def beginRecursionProtection(self, obj):
        ...
        if id(obj) in self.objectsStack:
            raise InvalidPlistException(
                "Recursive containers are not allowed in plists."
            )
    ...
    def computeOffsets(self, obj, asReference=False, isRoot=False):
        ...
        if obj is None:
            ...
            raise InvalidPlistException(
                "Unknown object type: %s (%s)" % (type(obj).__name__, repr(obj))
            )
    ...
    def writeObjectReference(self, obj, output):
        ...
        if position is None:
            ...
            output += self.binaryInt(
                len(self.writtenReferences) - 1, byteSize=self.trailer.objectRefSize
            )
            return (True, output)
        else:
    ...
    def writeObject(self, obj, output, setReferencePosition=False):
        ...
        def timedelta_total_seconds(td):
            ...
            return (
                td.microseconds + (td.seconds + td.days * 24 * 3600) * 10.0**6
            ) / 10.0**6
    ...
    def writeOffsetTable(self, output):
        ...
        for obj, order in writtenReferences:
            ...
            if position is None:
                raise InvalidPlistException(
                    "Error while writing offsets table. Object not found. %s" % obj
                )
    ...
    def binaryInt(self, obj, byteSize=None, as_number=False):
        ...
        if byteSize == 1:
            ...
            try:
                ...
                raise InvalidPlistException(
                    "Unable to pack integer %d: %s" % (obj, e)
                )
        else:
            raise InvalidPlistException(
                "Core Foundation can't handle integers with size greater than 16 bytes."
            )
        return result
