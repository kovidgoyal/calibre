...
add_matches(const word_trie *wt, char_type *codepoints, size_t *pos, const size_t sz) {
    ...
    for (size_t i = wt->match_offset + 1; i < wt->match_offset + 1 + num && *pos < sz; i++, (*pos)++) { codepoints[*pos] = mark_to_cp[mark_groups[i]]; }
}
...
codepoints_for_word(const char *word, size_t len) {
    ...
    for (size_t i = 0; i < cpos; i++) {
        PyObject *t = PyLong_FromUnsignedLong(codepoints[i]);
        if (t == NULL) {
            Py_DECREF(ans);
            return NULL;
        }
...
static PyMethodDef unicode_names_methods[] = {
    {"codepoints_for_word",
     ...
     "contains ``word``,"},
    {"name_for_codepoint", (PyCFunction)nfc, METH_VARARGS, "Returns the given codepoint's name"},
...
PyInit_unicode_names(void) { return PyModuleDef_Init(&module_def); }
