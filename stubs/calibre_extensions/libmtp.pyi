...
class Device:
    ...
    def get_filesystem(
        ...
        (
            ...
            ' callable that is called as with (entry, level). It is called with every found object. If callback returns False and the object is a folder, it is'
            ' not recursed into.'
        )
        pass
    ...
    def get_file(self, fileid: int, stream: object, callback: Callable[[int, int], object] | None = None) -> tuple[bool, list[tuple[int, str]]]:
        (
            'get_file(fileid, stream, callback=None) -> Get the file specified by fileid from the device. stream must be a file-like object. The file will be'
            ' written to it. callback works the same as in get_filelist(). Returns ok, errs, where errs is a list of errors (if any).'
        )
        pass

    def get_file_by_name(
        self, storage_id: int, parent_id: int, names: tuple[str, ...], stream: object, callback: Callable[[int, int], object] | None = None
        ...
        (
            ...
            ' Returns None or (ok, errs), where errs is a list of errors (if any).'
        )
        pass

    def list_folder_by_name(self, storage_id: int, parent_id: int, names: tuple[str, ...]) -> list[dict[str, int | str | bool]] | None:
        (
            'list_folder_by_name(storage_id, parent_id, names) -> List the folder specified by names (a tuple of name components) relative to parent_id from'
            ' the device. Return None or a list of entries.'
        )
        pass
    ...
    def put_file(
        self, storage_id: int, parent_id: int, filename: str, stream: object, size: int, callback: Callable[[int, int], object] | None = None
    ) -> tuple[dict[str, int | str | bool] | None, list[tuple[int, str]]]:
        (
            'put_file(storage_id, parent_id, filename, stream, size, callback=None) -> Put a file on the device. The file is read from stream. It is put inside'
            ' the folder identified by parent_id on the storage identified by storage_id. Use parent_id=0 to put it in the root. stream must be a file-like'
            ' object. size is the size in bytes of the data in stream. callback works the same as in get_filelist(). Returns fileinfo, errs, where errs is a'
            ' list of errors (if any), and fileinfo is a file information dictionary, as returned by get_filelist(). fileinfo will be None if case or errors.'
        )
        pass
    ...
    def storage_info(self) -> list[dict[str, int | str | bool]]:
        (
            'Information about the storage locations on the device. Returns a list of dictionaries where each dictionary corresponds to the'
